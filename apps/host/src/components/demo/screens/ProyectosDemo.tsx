...
import { demoProyectos } from '../mock/demoData';
  ...
  return (
    <ScreenShell icon={FolderKanban} titulo="Proyectos" aviso="Datos de demostración · sin conexión a backend">
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        {demoProyectos.map((p) => (
          <Bloque key={p.id} titulo={p.label} icon={FolderKanban} tag={<Tag>mock</Tag>}>
      ...
      <p className="mt-6 text-2xs uppercase tracking-widest text-amber-400/80">
        Mapa de proyectos demo · estados y métricas mock, no reales
      </p>
    </ScreenShell>
  );
}
