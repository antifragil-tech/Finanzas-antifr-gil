...
const MONEDAS = [
  { id: 'eur', label: 'EUR (€ 1,00)' },
  { id: 'usd', label: 'USD ($ 1,00)' },
  { id: 'gbp', label: 'GBP (£ 1,00)' },
];
const METODOS_PAGO = [
...
function calcLine(line: EditorLine) {
  ...
  const ivaAmt = +taxes
    .filter((t) => t.tipo === 'iva')
    .reduce((s, t) => s + (base * t.porcentaje) / 100, 0)
    .toFixed(2);
  const retAmt = +taxes
  ...
  return createPortal(
    <>
      ...
      <div
        ...
        className="fixed z-[9999] w-72 rounded-xl border border-white/10 bg-zinc-900 p-4 shadow-2xl"
      ...
      >
        ...
        <div className="mb-3 flex items-center gap-2">
          ...
          <div className="flex flex-1 items-center gap-1">
            <input
              type="number"
              value={nDias}
              onChange={(e) => setNDias(e.target.value)}
              onKeyDown={(e) => {
                if (e.key === 'Enter' && nDias) {
                  onChange(addDays(today, parseInt(nDias)));
                  onClose();
                }
              }}
              placeholder="N"
              className="h-7 w-10 rounded border border-white/10 bg-zinc-800 px-1.5 text-center text-xs text-white [color-scheme:dark] focus:outline-none"
            />
        ...
        <div className="mb-1 grid grid-cols-7">
          {DAY_NAMES.map((d) => (
            <div key={d} className="text-2xs py-0.5 text-center font-semibold text-zinc-600">
              {d}
            </div>
          ))}
        </div>
          ...
          {cells.map((day, i) =>
            day === null ? (
              ...
              <button
                ...
                type="button"
                onClick={() => {
                  onChange(new Date(yr, mo, day).toISOString().slice(0, 10));
                  onClose();
                }}
                className={`flex aspect-square items-center justify-center rounded-lg text-xs transition-colors ${isSelected(day) ? 'bg-blue-600 font-medium text-white' : 'text-zinc-400 hover:bg-white/[0.06] hover:text-white'}`}
              >
                {day}
              </button>
            ),
        ...
        <button
          type="button"
          className="text-2xs mt-3 w-full text-center text-blue-400 transition-colors hover:text-blue-300"
        >
...
function ContactoSearch({
  ...
  const filtered = useMemo(() => {
    ...
    return contactos.filter(
      (c) => c.nombre.toLowerCase().includes(q) || (c.nif ?? '').toLowerCase().includes(q),
    );
  }, [contactos, search]);
  ...
  return (
    <div ref={wrapRef}>
      <div
        onClick={openDrop}
        className={`flex h-10 cursor-pointer items-center gap-2 rounded-lg border px-3 transition-colors ${open ? 'border-blue-500/50 bg-zinc-800' : 'border-white/[0.08] bg-zinc-900/60 hover:border-white/15'}`}
      >
        <Search size={13} className="shrink-0 text-zinc-500" />
        <span
          className={`flex-1 truncate text-sm ${selected ? 'font-medium text-white' : 'text-zinc-500'}`}
        >
        ...
        createPortal(
          <>
            ...
            <div
              ...
              className="fixed z-[9999] overflow-hidden rounded-xl border border-white/10 bg-zinc-900 shadow-2xl"
            >
              <div className="border-b border-white/[0.06] p-2">
                <input
                  ...
                  className="w-full rounded-lg bg-zinc-800 px-3 py-1.5 text-sm text-white placeholder:text-zinc-500 focus:outline-none"
                  onMouseDown={(e) => e.stopPropagation()}
                />
              </div>
              <div className="max-h-56 overflow-y-auto py-1">
                {filtered.length === 0 ? (
                  <p className="px-3 py-3 text-center text-xs text-zinc-600">Sin resultados</p>
                ) : (
                  filtered.map((c) => (
                    <button
                      key={c.id}
                      ...
                      onMouseDown={(e) => {
                        e.stopPropagation();
                        onSelect(c.id, c.nombre, c.nif ?? '', c.direccion ?? '');
                        setOpen(false);
                        setSearch('');
                      }}
                    ...
                    >
                      ...
                      {c.nif && <p className="text-2xs font-mono text-zinc-500">{c.nif}</p>}
                    </button>
                  ))
                )}
              </div>
...
function ImpuestosCell({
  ...
  return (
    <div ref={wrapRef} className="flex w-full min-w-0 items-center gap-1">
      <button
        type="button"
        onClick={() => {
          if (scrollRef.current) scrollRef.current.scrollLeft -= 60;
        }}
      ...
      <div
        ref={scrollRef}
        onClick={openDrop}
        className={`flex min-h-[26px] flex-1 cursor-pointer items-center gap-1 overflow-x-hidden px-0.5`}
      >
        {selected.length === 0 ? (
          ...
          selected.map((t) => (
            <span
              key={t.id}
              className={`text-2xs inline-flex shrink-0 items-center gap-0.5 whitespace-nowrap rounded px-1.5 py-0.5 font-medium ${tagColor[t.tipo]}`}
            >
      ...
      {open &&
        createPortal(
          <>
            ...
            <div
              style={{ top: pos.top, left: pos.left, width: pos.width }}
              className="fixed z-[9999] max-h-[300px] overflow-y-auto rounded-xl border border-white/10 bg-zinc-900 py-1 shadow-2xl"
              onMouseDown={(e) => e.stopPropagation()}
            >
              {IMPUESTOS_CAT.map((g) => (
                <div key={g.grupo}>
                  <p className="text-2xs px-3 pb-1 pt-2 font-semibold uppercase tracking-widest text-zinc-600">
                    {g.grupo}
                  </p>
                  {g.items.map((t) => {
                    ...
                    return (
                      <button
                        key={t.id}
                        type="button"
                        onMouseDown={() => toggle(t.id)}
                        className="flex w-full items-center gap-2 px-3 py-1.5 text-xs transition-colors hover:bg-white/[0.04]"
                      >
                        <div
                          className={`flex h-3.5 w-3.5 shrink-0 items-center justify-center rounded border ${active ? 'border-blue-500 bg-blue-600' : 'border-white/20'}`}
                        >
                          {active && <Check size={9} className="text-white" />}
                        </div>
                        <span className={active ? 'font-medium text-white' : 'text-zinc-400'}>
                          {t.nombre}
                        </span>
                      </button>
                    );
  ...
  return (
    <>
      <button
        ...
        className={`flex h-9 w-full items-center justify-between rounded-lg border px-3 text-sm transition-colors ${open ? 'border-blue-500/50 bg-zinc-800 text-white' : 'border-white/[0.08] bg-zinc-900/60 text-zinc-300 hover:border-white/15'} ${disabled ? 'pointer-events-none opacity-40' : ''}`}
      >
        <span className="truncate">{sel?.label ?? '—'}</span>
        <ChevronDown size={12} className="ml-2 shrink-0 text-zinc-500" />
      </button>
      {open &&
        createPortal(
          <>
            ...
            <div
              style={{ top: pos.top, left: pos.left, width: pos.width }}
              className="fixed z-[9999] max-h-[200px] overflow-y-auto rounded-lg border border-white/10 bg-zinc-900 py-1 shadow-2xl"
            >
              {options.map((o) => (
                <button
                  key={o.id}
                  ...
                  className={`w-full px-3 py-2 text-left text-sm transition-colors ${o.id === value ? 'bg-white/[0.08] font-medium text-white' : 'text-zinc-300 hover:bg-white/[0.04] hover:text-white'}`}
                >
                  {o.label}
                </button>
              ))}
...
function OpcionesDrawer({
  ...
  return createPortal(
    <>
      ...
      <div className="fixed right-0 top-0 z-[201] flex h-full w-80 flex-col border-l border-white/10 bg-zinc-950 shadow-2xl">
        ...
        <div className="flex-1 space-y-5 overflow-y-auto px-5 py-5">
          <div>
            ...
            <PortalSelect
              value={serie}
              onChange={setSerie}
              options={SERIES_CONFIG.map((s) => ({ id: s.id, label: s.label }))}
            />
          </div>
          ...
          <div>
            ...
            <PortalSelect
              value={idioma}
              onChange={setIdioma}
              options={IDIOMAS.map((i) => ({ id: i.id, label: i.label }))}
            />
          </div>
          ...
          <div>
            ...
            <PortalSelect
              value={moneda}
              onChange={setMoneda}
              options={MONEDAS.map((m) => ({ id: m.id, label: m.label }))}
            />
          </div>
          ...
          <div>
            ...
            <PortalSelect
              value="default"
              onChange={() => {}}
              options={[{ id: 'default', label: 'Plantilla por defecto' }]}
            />
          </div>

          <div className="flex items-center gap-2 pt-1">
            <input type="checkbox" id="cuentas-all" className="rounded" disabled />
            <label htmlFor="cuentas-all" className="text-xs text-zinc-500">
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {['Clásica', 'Moderna', 'Minimalista'].map((t, i) => (
                <div
                  key={t}
                  className={`flex aspect-[0.7] cursor-pointer items-end rounded-lg border-2 p-2 transition-colors ${i === 0 ? 'border-blue-500 bg-blue-500/5' : 'border-white/[0.08] bg-zinc-900/40 hover:border-white/20'}`}
                >
                  <span className="text-2xs leading-tight text-zinc-500">{t}</span>
                </div>
              ))}
            </div>
            <button
              type="button"
              className="text-2xs mt-2 text-blue-400 transition-colors hover:text-blue-300"
            >
...
function buildInvoiceHtml(p: Omit<PreviewProps, 'isPrivateMode' | 'onClose'>): string {
  ...
  const summaryRows = [
    ...
    p.totals.ivaTotal !== 0
      ? `<tr><td style="padding:6px 0;font-size:12px;color:#6b7280">IVA</td><td style="padding:6px 0;font-size:12px;color:#374151;text-align:right;font-family:'Courier New',monospace">${fmt(p.totals.ivaTotal)}</td></tr>`
      : '',
    p.totals.retTotal !== 0
      ? `<tr><td style="padding:6px 0;font-size:12px;color:#6b7280">Retención</td><td style="padding:6px 0;font-size:12px;color:#dc2626;text-align:right;font-family:'Courier New',monospace">- ${fmt(Math.abs(p.totals.retTotal))}</td></tr>`
      : '',
  ]
    .filter(Boolean)
    .join('');
...
function FacturaPreview({
  ...
  return createPortal(
    <div className="fixed inset-0 z-[300] flex bg-black/60 backdrop-blur-sm">
      ...
      <button
        onClick={onClose}
        className="absolute right-4 top-4 z-10 rounded-lg border border-white/10 bg-zinc-800 p-2 text-zinc-300 transition-colors hover:bg-zinc-700"
      >
        <X size={16} />
      </button>
      <div className="flex flex-1 justify-center overflow-y-auto px-4 py-16">
        <div className="flex min-h-[1122px] w-full max-w-[760px] flex-col overflow-hidden rounded bg-white font-sans shadow-2xl">
          {/* Header band */}
          <div className="flex items-start justify-between bg-zinc-900 px-10 py-8">
            <div>
              ...
              {emisorEmail && <p className="mt-0.5 text-xs text-zinc-400">{emisorEmail}</p>}
            </div>
            <div className="text-right">
              <p className="text-4xl font-light leading-none tracking-tight text-white">Factura</p>
              <div className="mt-3 space-y-1">
                <p className="text-xs text-zinc-400">
                  Nº <span className="font-mono font-medium text-white">{numero || '—'}</span>
                </p>
                <p className="text-xs text-zinc-400">
                ...
                {vencimiento && (
                  <p className="text-xs text-zinc-400">
                    Vencimiento: <span className="font-semibold text-white">{vencimiento}</span>
                  </p>
                )}
              </div>
            </div>
          ...
          {/* Bill to + total due */}
          <div className="flex items-start justify-between border-b border-zinc-100 px-10 py-7">
            <div>
              <p className="text-2xs mb-2 font-medium uppercase tracking-widest text-zinc-400">
                Facturado a
              </p>
              <p className="text-base font-medium text-zinc-900">{clienteNombre || '—'}</p>
              {clienteNif && !isPrivateMode && (
                <p className="mt-1 text-xs text-zinc-500">NIF / CIF: {clienteNif}</p>
              )}
            </div>
            <div className="text-right">
              <p className="text-2xs mb-1.5 font-medium uppercase tracking-widest text-zinc-400">
                Total a cobrar
              </p>
              <p className="text-3xl font-light tracking-tight text-zinc-900">
                {fmt(totals.totalACobrar)}
              </p>
            </div>
          </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b-2 border-zinc-200 bg-zinc-50">
                <th className="text-2xs px-4 py-3 text-left font-semibold uppercase tracking-widest text-zinc-400">
                  Descripción del artículo
                </th>
                <th className="text-2xs w-28 px-4 py-3 text-right font-semibold uppercase tracking-widest text-zinc-400">
                  Precio
                </th>
                <th className="text-2xs w-16 px-4 py-3 text-center font-semibold uppercase tracking-widest text-zinc-400">
                  Cant.
                </th>
                <th className="text-2xs w-28 px-4 py-3 text-right font-semibold uppercase tracking-widest text-zinc-400">
                  Total
              ...
              {lineasCalc.length === 0 ? (
                <tr>
                  <td colSpan={4} className="py-12 text-center text-sm text-zinc-400">
                    Sin líneas de concepto
                  </td>
                </tr>
              ) : (
                lineasCalc.map((l, i) =>
                  l.type === 'titulo' ? (
                    <tr key={i} className="border-t border-zinc-100 bg-zinc-50">
                      <td
                        colSpan={4}
                        className="text-2xs px-4 py-3 font-semibold uppercase tracking-widest text-zinc-500"
                      >
                        {l.concepto}
                      </td>
                    </tr>
                    ...
                    >
                      <td className="px-4 py-4">
                        ...
                        {l.descripcion && (
                          <p className="mt-0.5 text-xs text-zinc-400">{l.descripcion}</p>
                        )}
                      </td>
                      ...
                      <td className="px-4 py-4 text-right font-mono text-sm font-medium text-zinc-900">
                        {fmt(l.base)}
                      </td>
                    </tr>
                  ),
          ...
          {/* Totals */}
          <div className="flex justify-end border-t-2 border-zinc-200 px-10 py-8">
            <div className="w-64 space-y-2">
              ...
              <div className="flex justify-between text-base font-semibold text-zinc-900">
                ...
                <span className="font-mono">{fmt(totals.totalACobrar)}</span>
              </div>
            </div>
          </div>
          ...
          {metodoPago === 'transfer' && cuentaIban && (
            <div className="border-t border-zinc-100 bg-zinc-50 px-10 py-5">
              <p className="text-2xs mb-3 font-semibold uppercase tracking-widest text-zinc-400">
                Datos para el pago
              </p>
              <div className="space-y-1">
                ...
                <p className="font-mono text-sm font-medium tracking-wider text-zinc-900">
                  {cuentaIban.replace(/(.{4})/g, '$1 ').trim()}
                </p>
...
export function FacturaEmitidaEditor({
  ...
  useEffect(() => {
    ...
    void getCuentasBancarias(sociedadId)
      .then(setCuentas)
      .catch(() => {});
  }, [sociedadId]);
  ...
  const totals = useMemo(() => {
    ...
    const descImporte =
      descGlobalActivo && dv > 0
        ? descModo === 'pct'
          ? +((subtotalBruto * dv) / 100).toFixed(2)
          : +dv.toFixed(2)
        : 0;
  ...
  async function handleSave(estado: EstadoFacturaEmitida) {
    ...
    try {
      const lineasPayload: LineaFactura[] = lineasCalc
        ...
        .map((l) => {
          const primerIva = IMPUESTOS_FLAT.find(
            (t) => l.impuestos.includes(t.id) && t.tipo === 'iva',
          );
          return {
      ...
      const primerIva = IMPUESTOS_FLAT.find(
        (t) => lineas.some((l) => l.impuestos.includes(t.id)) && t.tipo === 'iva',
      );
      const primerRet = IMPUESTOS_FLAT.find(
        (t) => lineas.some((l) => l.impuestos.includes(t.id)) && t.tipo === 'retencion',
      );
      ...
      const cuentaSeleccionada =
        metodoPago === 'transfer' ? (cuentas.find((c) => c.iban === cuentaId) ?? null) : null;
  ...
  return (
    <div className="space-y-5 pb-16">
      {/* Error banner */}
      {error && (
        <div className="bg-rose-500/8 flex shrink-0 items-center gap-2 rounded-xl border border-rose-500/20 px-4 py-2.5">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <button
          onClick={onBack}
          className="flex items-center gap-1.5 rounded-xl border border-white/[0.08] px-3 py-2 text-xs font-medium text-zinc-500 transition-all hover:border-white/15 hover:text-zinc-200"
        >
          <ArrowLeft size={13} />
          Volver
        </button>
        ...
        <div className="ml-auto flex items-center gap-2">
          <button
            type="button"
            title="Plantillas"
            className="rounded-xl border border-white/[0.08] p-2 text-zinc-500 transition-all hover:border-white/15 hover:text-zinc-300"
          >
            <BookOpen size={14} />
          </button>
          <button
            ...
            className={`flex items-center gap-1.5 rounded-xl border px-3 py-2 text-xs font-medium transition-all ${showPreview ? 'border-white/15 bg-white/5 text-zinc-200' : 'border-white/[0.08] text-zinc-500 hover:border-white/15 hover:text-zinc-300'}`}
          >
            <Eye size={13} />
            Vista previa
          </button>
          <button
            ...
            className={`flex items-center gap-1.5 rounded-xl border px-3 py-2 text-xs font-medium transition-all ${showOpciones ? 'border-white/15 bg-white/5 text-zinc-200' : 'border-white/[0.08] text-zinc-500 hover:border-white/15 hover:text-zinc-300'}`}
          >
          ...
          <button
            type="button"
            onClick={() => {
              void handleSave('borrador');
            }}
            disabled={saving}
            className="flex items-center gap-1.5 rounded-xl border border-white/[0.08] px-3 py-2 text-xs font-medium text-zinc-400 transition-all hover:border-white/15 hover:text-zinc-200 disabled:opacity-50"
          >
          ...
          <div ref={approveBtnRef} className="flex items-center">
            <button
              type="button"
              onClick={() => {
                void handleSave('emitida');
              }}
              disabled={saving}
              className="flex items-center gap-1.5 rounded-l-xl bg-blue-600 py-2 pl-4 pr-3 text-xs font-medium text-white transition-colors hover:bg-blue-500 disabled:opacity-50"
            >
              <Lock size={13} />
              Aprobar
            </button>
            <button
              type="button"
              onClick={openApproveMenu}
              className="flex items-center rounded-r-xl border-l border-blue-500/30 bg-blue-600 px-2 py-2 text-white transition-colors hover:bg-blue-500"
            >
              <ChevronDown size={13} />
            </button>
          </div>
      ...
      {showApproveMenu &&
        createPortal(
          <>
            ...
            <div
              style={{ top: approveMenuPos.top, right: approveMenuPos.right }}
              className="fixed z-[9999] w-48 rounded-xl border border-white/10 bg-zinc-900 py-1 shadow-2xl"
            >
              <button
                onMouseDown={() => {
                  void handleSave('emitida');
                ...
                className="w-full px-3 py-2 text-left text-sm text-zinc-300 hover:bg-white/[0.04]"
              >
              ...
              <button
                ...
                className="w-full px-3 py-2 text-left text-sm text-rose-400 hover:bg-white/[0.04]"
              >
                Cancelar
              </button>
            </div>
      ...
      <div className="rounded-2xl border border-white/[0.06] bg-zinc-900/40 p-5">
        <div className="grid grid-cols-4 gap-5">
          ...
          <div>
            ...
            <ContactoSearch
              contactos={contactos}
              selectedId={contactoId}
              onSelect={(id, nombre, nif, dir) => {
                setContactoId(id);
          ...
          <div>
            ...
            <input
              ...
              className="h-10 w-full rounded-lg border border-white/[0.08] bg-zinc-900/60 px-3 font-mono text-sm text-white transition-colors [color-scheme:dark] hover:border-white/15 focus:border-blue-500/50 focus:outline-none"
            />
            <p className="text-2xs mt-1 text-zinc-600">
              Auto-calculado · editable si necesitas ajustarlo
            </p>
          </div>
          {/* Fecha */}
          <div>
            ...
            <input
              ...
              className="h-10 w-full rounded-lg border border-white/[0.08] bg-zinc-900/60 px-3 text-sm text-white transition-colors [color-scheme:dark] hover:border-white/15 focus:border-blue-500/50 focus:outline-none"
            />
          </div>
          {/* Vencimiento */}
          <div>
            ...
            <button
              ref={vencBtnRef}
              type="button"
              onClick={() => setShowVencPop((v) => !v)}
              className={`flex h-10 w-full items-center justify-between rounded-lg border px-3 text-sm transition-colors ${vencimiento ? 'text-white' : 'text-zinc-500'} ${showVencPop ? 'border-blue-500/50 bg-zinc-800' : 'border-white/[0.08] bg-zinc-900/60 hover:border-white/15'}`}
            >
            ...
            {showVencPop && (
              <VencimientoPopover
                ...
                onClose={() => setShowVencPop(false)}
                anchorRef={vencBtnRef}
              />
            )}
          </div>
        </div>
      ...
      {/* ── TABLA DE LÍNEAS ───────────────────────────────────────────────── */}
      <div className="overflow-visible rounded-2xl border border-white/[0.06] bg-zinc-900/40">
          ...
          <table className="w-full text-sm" style={{ minWidth: 800 }}>
            ...
            <thead>
              <tr className="border-b border-white/[0.06]">
                <th className="py-3" />
                <th className="text-2xs px-3 py-3 text-left font-semibold uppercase tracking-widest text-zinc-500">
                  Concepto
                </th>
                <th className="text-2xs px-2 py-3 text-left font-semibold uppercase tracking-widest text-zinc-500">
                  Descripción
                </th>
                <th className="py-3">
                  <button
                    ...
                    className="text-2xs mx-auto flex items-center gap-1 font-semibold uppercase tracking-widest text-zinc-500 transition-colors hover:text-zinc-300"
                  >
                    Cantidad <ChevronDown size={9} />
                  </button>
                </th>
                <th className="text-2xs px-2 py-3 text-right font-semibold uppercase tracking-widest text-zinc-500">
                  Precio
                </th>
                <th className="text-2xs px-2 py-3 text-left font-semibold uppercase tracking-widest text-zinc-500">
                  Impuestos
                </th>
                <th className="text-2xs px-3 py-3 text-right font-semibold uppercase tracking-widest text-zinc-500">
                  Total
                </th>
                <th className="py-3" />
              </tr>
                    ...
                    >
                      ...
                      <td colSpan={6} className="px-3 py-2">
                        <input
                          ...
                          className="w-full bg-transparent text-sm font-medium text-zinc-300 placeholder:text-zinc-600 focus:outline-none"
                      ...
                      <td className="pr-2 text-center">
                        <button
                          type="button"
                          onClick={() =>
                            setLineas((prev) => prev.filter((l) => l._key !== line._key))
                          }
                          className="text-zinc-600 opacity-0 transition-colors hover:text-rose-400 group-hover:opacity-100"
                        >
                          <Trash2 size={12} />
                        </button>
                      </td>
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-3 py-2">
                      ...
                      {line.type === 'suplido' && (
                        <span className="text-2xs mt-0.5 inline-block rounded bg-zinc-700/50 px-1.5 py-0.5 font-medium text-zinc-400">
                          Suplido
                        </span>
                      )}
                    </td>
                    ...
                    <td className="px-2 py-2">
                      <textarea
                        ...
                        rows={1}
                        className="min-h-[24px] w-full resize-y bg-transparent text-xs text-zinc-400 placeholder:text-zinc-700 focus:outline-none"
                        placeholder="Desc"
                      />
                    </td>
                    {/* Cantidad */}
                    <td className="px-2 py-2">
                      {modoDoc !== 'ocultar_u' && (
                        <input
                          ...
                          value={line.cantidad}
                          onChange={(e) =>
                            updateLinea(line._key, { cantidad: parseFloat(e.target.value) || 0 })
                          }
                          className="w-full bg-transparent text-center text-sm text-white [color-scheme:dark] focus:outline-none"
                        />
                      )}
                    </td>
                    {/* Precio */}
                    <td className="px-2 py-2">
                      <input
                        ...
                        value={line.precio}
                        onChange={(e) =>
                          updateLinea(line._key, { precio: parseFloat(e.target.value) || 0 })
                        }
                        className="w-full bg-transparent text-right text-sm text-white [color-scheme:dark] focus:outline-none"
                      />
                    </td>
                    {/* Impuestos */}
                    <td className="px-2 py-2">
                      {modoDoc !== 'sin_imp' && (
                        <ImpuestosCell
                          selectedIds={line.impuestos}
                          onChange={(ids) => updateLinea(line._key, { impuestos: ids })}
                        />
                      )}
                    </td>
                    {/* Total */}
                    ...
                    <td className="pr-2 text-center">
                      <button
                        ...
                        className="text-zinc-700 opacity-0 transition-colors hover:text-rose-400 group-hover:opacity-100"
                      >
                        <Trash2 size={12} />
                      </button>
                    </td>
        ...
        {showModoDoc &&
          createPortal(
            <>
              ...
              <div
                className="fixed z-[9999] w-64 rounded-xl border border-white/10 bg-zinc-900 p-4 shadow-2xl"
              ...
              >
                ...
                <div className="mb-3 space-y-1">
                  {MODOS_DOC.map((m) => (
                    <button
                      key={m.id}
                      type="button"
                      onClick={() => setModoDoc(m.id)}
                      className={`w-full rounded-lg px-2.5 py-1.5 text-left text-xs transition-colors ${modoDoc === m.id ? 'bg-blue-600/20 font-medium text-blue-300' : 'text-zinc-400 hover:bg-white/[0.04] hover:text-zinc-200'}`}
                    >
                      {m.label}
                    </button>
                  ))}
                ...
                <label className="flex cursor-pointer items-center gap-2">
                  <input
                    type="checkbox"
                    checked={mostrarUnidad}
                    onChange={(e) => setMostrarUnidad(e.target.checked)}
        ...
        <div className="flex items-center justify-between border-t border-white/[0.04] px-4 py-3">
          <div className="flex items-center">
            <button
              ...
              className="flex items-center gap-1.5 rounded-l-lg border border-white/[0.08] px-3 py-1.5 text-xs font-medium text-zinc-400 transition-all hover:border-white/15 hover:text-zinc-200"
            >
              <Plus size={12} />
              Añadir línea
            </button>
            <div className="relative">
              <button
                ...
                type="button"
                onClick={openAddMenu}
                className="flex items-center rounded-r-lg border border-l-0 border-white/[0.08] px-2 py-1.5 text-xs text-zinc-500 transition-all hover:border-white/15 hover:text-zinc-300"
              >
                <ChevronDown size={12} />
              </button>
              {showAddMenu &&
                createPortal(
                  <>
                    <div
                      ...
                      onMouseDown={() => setShowAddMenu(false)}
                    />
                    <div
                      className="fixed z-[9999] w-44 rounded-xl border border-white/10 bg-zinc-900 py-1 shadow-2xl"
                      style={{ top: addMenuPos.top, left: addMenuPos.left }}
                      onMouseDown={(e) => e.stopPropagation()}
                    >
                      <button
                        onMouseDown={() => {
                          setLineas((prev) => [...prev, emptyLine('titulo')]);
                      ...
                      <button
                        onMouseDown={() => {
                          setLineas((prev) => [...prev, emptyLine('suplido')]);
                          setShowAddMenu(false);
                        }}
                        className="w-full px-3 py-2 text-left text-sm text-zinc-300 hover:bg-white/[0.04]"
                      >
      ...
      <div className="space-y-3">
        ...
        <label className="flex cursor-not-allowed items-center gap-3 opacity-50">
          ...
          <span className="text-2xs rounded-full border border-amber-500/20 bg-amber-500/15 px-2 py-0.5 font-medium text-amber-400">
            Mejorar plan
          </span>
        </label>
        ...
        <div>
          <label className="flex cursor-pointer items-center gap-3">
            <input
              type="checkbox"
              checked={showTextoDoc}
              onChange={(e) => setShowTextoDoc(e.target.checked)}
              className="rounded accent-blue-500"
            />
            <span className="text-sm text-zinc-400">Añadir texto en el documento</span>
          </label>
          {showTextoDoc && (
            <div className="mt-3 overflow-hidden rounded-xl border border-white/[0.06] bg-zinc-900/40">
              {/* Toolbar */}
              <div className="flex flex-wrap items-center gap-1 border-b border-white/[0.06] px-3 py-2">
                {[
                  ...
                  <button
                    ...
                    className="rounded p-1.5 text-zinc-500 transition-colors hover:bg-white/5 hover:text-zinc-200"
                  >
                    <Icon size={13} />
                  </button>
                ))}
                ...
                <div className="mx-1 h-4 w-px bg-white/10" />
                <select
                  className="bg-transparent text-xs text-zinc-500 [color-scheme:dark] focus:outline-none"
                  onChange={(e) => execFmt('fontSize', e.target.value)}
                >
                  {[10, 12, 14, 16, 18, 20, 24].map((s) => (
                    <option key={s} value={String(s / 4)}>
                      {s}
                    </option>
                  ))}
                </select>
              ...
              <div
                ...
                className="min-h-[80px] px-4 py-3 text-sm text-zinc-200 focus:outline-none"
                style={{ lineHeight: 1.6 }}
              />
              <div className="border-t border-white/[0.04] px-4 py-2">
                <button
                  type="button"
                  className="text-2xs text-blue-400 transition-colors hover:text-blue-300"
                >
        ...
        <div>
          <label className="flex cursor-pointer items-center gap-3">
            <input
              type="checkbox"
              checked={showMensaje}
              onChange={(e) => setShowMensaje(e.target.checked)}
              className="rounded accent-blue-500"
            />
          ...
          {showMensaje && (
            <textarea
              value={mensajeFinal}
              onChange={(e) => setMensajeFinal(e.target.value)}
              rows={3}
              className="mt-3 w-full resize-y rounded-xl border border-white/[0.06] bg-zinc-900/40 px-4 py-3 text-sm text-zinc-200 transition-colors placeholder:text-zinc-600 focus:border-white/15 focus:outline-none"
      ...
      <div className="flex justify-end">
        <div className="w-72 space-y-2 rounded-2xl border border-white/[0.06] bg-zinc-900/40 p-5">
          ...
          {showDescuento && (
            <div className="space-y-2 border-b border-white/[0.06] pb-2">
              ...
              <label className="flex cursor-not-allowed items-center justify-between text-xs text-zinc-500 opacity-50">
              ...
              <label className="flex cursor-pointer items-center justify-between gap-2 text-xs">
                <span className="flex items-center gap-1.5 text-zinc-400">
                  <input
                    type="checkbox"
                    checked={descGlobalActivo}
                    onChange={(e) => setDescGlobalActivo(e.target.checked)}
                    className="rounded accent-blue-500"
                  />
                  Descuento global
                </span>
                {descGlobalActivo && (
                  <div className="flex items-center gap-1">
                    <input
                      ...
                      className="w-16 rounded border border-white/10 bg-zinc-800 px-2 py-0.5 text-right font-mono text-xs text-white [color-scheme:dark] focus:outline-none"
                    />
                    <button
                      type="button"
                      onClick={() => setDescModo((m) => (m === 'pct' ? 'fijo' : 'pct'))}
                      className="text-2xs font-medium text-blue-400 transition-colors hover:text-blue-300"
                    >
                      {descModo === 'pct' ? '%' : '€'}
                    </button>
                    <span className="font-mono text-xs text-zinc-600">
                      -{fmt(totals.descImporte)}
                    </span>
                  </div>
                )}
              </label>
          ...
          <div className="flex justify-between border-t border-white/[0.08] pt-2 text-base font-semibold text-white">
            <span>Total</span>
      ...
      <div className="grid grid-cols-2 gap-5">
        ...
        <div className="space-y-4 rounded-2xl border border-white/[0.06] bg-zinc-900/40 p-5">
          ...
          {metodoPago === 'transfer' && (
            <div className="space-y-2.5">
              ...
              {cuentas.length > 0 && (
                <PortalSelect
                  ...
                  options={[
                    ...
                    ...cuentas.map((c) => ({
                      id: c.iban,
                      label: `${c.alias}  ·  ${c.iban.replace(/(.{4})/g, '$1 ').trim()}`,
                    })),
                  ]}
                />
              )}
              {cuentas.length === 0 && !showAddCuenta && (
                <p className="text-xs text-zinc-600">
                  No hay cuentas registradas para esta sociedad.
                </p>
              )}
              {!showAddCuenta ? (
                <button
                  type="button"
                  onClick={() => setShowAddCuenta(true)}
                ...
                <div className="space-y-2 rounded-xl border border-white/[0.08] bg-zinc-800/60 p-3">
                  <p className="text-2xs font-semibold uppercase tracking-widest text-zinc-500">
                    Nueva cuenta bancaria
                  </p>
                  <input
                    value={ncAlias}
                    onChange={(e) => setNcAlias(e.target.value)}
                    placeholder="Alias (ej. BBVA Principal)"
                    className="h-8 w-full rounded-lg border border-white/[0.08] bg-zinc-900/60 px-3 text-xs text-white focus:outline-none"
                  />
                  <input
                    value={ncTitular}
                    onChange={(e) => setNcTitular(e.target.value)}
                    placeholder="Titular de la cuenta"
                    className="h-8 w-full rounded-lg border border-white/[0.08] bg-zinc-900/60 px-3 text-xs text-white focus:outline-none"
                  />
                  <input
                    value={ncIban}
                    onChange={(e) => setNcIban(e.target.value)}
                    placeholder="IBAN (ES00 0000 0000 0000 0000 0000)"
                    className="h-8 w-full rounded-lg border border-white/[0.08] bg-zinc-900/60 px-3 font-mono text-xs text-white focus:outline-none"
                  />
                  <div className="grid grid-cols-2 gap-2">
                    <input
                      value={ncBanco}
                      onChange={(e) => setNcBanco(e.target.value)}
                      placeholder="Banco (opcional)"
                      className="h-8 rounded-lg border border-white/[0.08] bg-zinc-900/60 px-3 text-xs text-white focus:outline-none"
                    />
                    <input
                      value={ncSwift}
                      onChange={(e) => setNcSwift(e.target.value)}
                      placeholder="SWIFT/BIC (opcional)"
                      className="h-8 rounded-lg border border-white/[0.08] bg-zinc-900/60 px-3 font-mono text-xs text-white focus:outline-none"
                    />
                  </div>
                  <div className="flex gap-2 pt-1">
                    <button
                      type="button"
                      onClick={() => {
                        void handleAddCuenta();
                      }}
                      disabled={savingCuenta || !ncAlias || !ncTitular || !ncIban}
                      className="h-8 flex-1 rounded-lg bg-blue-600 text-xs font-medium text-white transition-colors hover:bg-blue-500 disabled:opacity-40"
                    >
          ...
          {metodoPago !== 'transfer' && (
            <div className="bg-blue-500/6 flex items-center gap-3 rounded-xl border border-blue-500/15 p-3">
              <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-blue-500/15">
                <CreditCard size={14} className="text-blue-400" />
              </div>
              <div className="min-w-0 flex-1">
                <p className="text-2xs leading-snug text-zinc-400">
                  Conecta tu pasarela de pago para cobrar online de forma rápida
                </p>
              </div>
              <button
                type="button"
                className="text-2xs shrink-0 font-medium text-blue-400 transition-colors hover:text-blue-300"
              >
                Conectar
              </button>
            </div>
        ...
        <div className="space-y-4 rounded-2xl border border-white/[0.06] bg-zinc-900/40 p-5">
          ...
          <div>
            ...
            <PortalSelect
              ...
              options={CUENTAS_CONTABLES.map((c) => ({
                id: c.codigo,
          ...
          <div>
            ...
            <div
              className="flex min-h-[36px] cursor-text flex-wrap gap-1 rounded-lg border border-white/[0.08] bg-zinc-900/60 px-3 py-1.5 hover:border-white/15"
              onClick={() => document.getElementById('tag-input')?.focus()}
            >
              {tags.map((t) => (
                <span
                  key={t}
                  className="text-2xs inline-flex items-center gap-1 rounded-md bg-zinc-700/60 px-1.5 py-0.5 font-medium text-zinc-300"
                >
                  {t}
                  <button
                    type="button"
              ...
              <input
                ...
                className="min-w-[60px] flex-1 bg-transparent py-0.5 text-xs text-white placeholder:text-zinc-600 focus:outline-none"
          ...
          <div>
            ...
            <input
              ...
              className="h-8 w-full rounded-lg border border-white/[0.08] bg-zinc-900/60 px-3 text-sm text-white transition-colors hover:border-white/15 focus:outline-none"
          ...
          <div>
            ...
            <input
              className="h-8 w-full rounded-lg border border-white/[0.08] bg-zinc-900/60 px-3 text-sm text-zinc-400 transition-colors hover:border-white/15 focus:outline-none"
      ...
      {showPreview &&
        (() => {
          ...
          const cuentaSel =
            metodoPago === 'transfer' ? cuentas.find((c) => c.iban === cuentaId) : undefined;
          return (
            <FacturaPreview
              numero={numero}
              serie={serie}
              ...
              emisorLocalidad={
                soc
                  ? `${soc.localidad ?? ''}${soc.codigo_postal ? ' ' + soc.codigo_postal : ''}`.trim()
                  : ''
              }
