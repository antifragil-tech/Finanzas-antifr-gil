...
function KpiCard({
  ...
  return (
    <button
      onClick={onClick}
      className={`group w-full rounded-2xl border border-white/5 bg-zinc-900/40 p-5 text-left transition-all hover:bg-zinc-900/70 ${onClick ? 'cursor-pointer' : 'cursor-default'}`}
    >
...
export function DashboardContabilidad({ isPrivateMode, onNavigate }: Props) {
  ...
  const facturasVencidas = facturas.filter(
    (f) => f.estado === 'pendiente_pago' && f.fecha_vencimiento && f.fecha_vencimiento < hoy,
  );
              ...
              <tbody className="divide-y divide-white/[0.04]">
                {recent.map((m) => (
                  <tr key={m.id} className="transition-colors hover:bg-white/[0.02]">
                    ...
                    <td className="max-w-xs truncate px-4 py-3 text-xs text-zinc-200">
                      {m.concepto}
                    </td>
                    <td className="px-4 py-3">
                      <span
                        className={`text-2xs rounded-full px-2 py-0.5 font-medium ${categoriaColor(m.categoria)}`}
                      >
                    ...
                    >
                      {isPrivateMode
                        ? '****'
                        : `${m.importe >= 0 ? '+' : ''}${m.importe.toLocaleString('es-ES', { minimumFractionDigits: 2 })} €`}
                    </td>
                    <td className="px-4 py-3 text-center">
                      <span
                        className={`inline-block h-2 w-2 rounded-full ${m.revisado ? 'bg-emerald-500' : 'bg-zinc-700'}`}
                      />
                    </td>
                  </tr>
                ))}
            ...
            {facturasVencidas.map((f) => (
              <div key={f.id} className="flex items-center justify-between text-xs">
                ...
                <span className="font-mono font-medium text-rose-300">
                  {fmt(f.total_a_pagar, isPrivateMode)}
                </span>
                <span className="text-2xs rounded-full bg-rose-500/20 px-2 py-0.5 font-medium text-rose-400">
                  VENCIDA
                </span>
              </div>
            ))}
