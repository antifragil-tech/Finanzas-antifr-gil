...
describe('resolveFacturaPdfSource', () => {
  it('con storage_path → fuente firmada (signed)', () => {
    expect(
      resolveFacturaPdfSource({ storage_path: 'recibidas/abc/f.pdf', archivo_url: null }),
  ...
  it('solo archivo_url (legado) → fallback directo (legacy)', () => {
    expect(
      resolveFacturaPdfSource({ storage_path: null, archivo_url: 'https://x/old.pdf' }),
    ).toEqual({ kind: 'legacy', url: 'https://x/old.pdf' });
  });

  it('sin storage_path ni archivo_url → none', () => {
    expect(resolveFacturaPdfSource({ storage_path: null, archivo_url: null })).toEqual({
      kind: 'none',
    });
  });
});
