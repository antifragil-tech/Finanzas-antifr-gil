...
export function CalendarioSpike() {
  ...
  // ── Reparto en carriles (anti-solape) ─────────────────────────────────────
  // DayPilot Lite, cuando dos citas coinciden en hora, ENSANCHA la cita que
  // empieza antes para rellenar el hueco que queda libre después y deja que la
  // siguiente se le monte encima (no hay opción de config para evitarlo). Aquí,
  // tras cada render, reescribimos left/width de cada cita: por columna, las
  // citas que se solapan en el tiempo se reparten en carriles iguales
  // (100/nCarriles %) y NUNCA se pisan. Si no caben, se estrechan (lo pedido).
  const repartirCarriles = () => {
  ...
  // DayPilot llama onAfterEventRender una vez por cita; agrupamos en un solo
