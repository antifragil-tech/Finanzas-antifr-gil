...
import { crearCitasMock, VIVOFACIL_VALOR_SESION } from '../spike/mockData';
...
export function Vivofacil() {
  ...
  const citas = crearCitasMock(hoy).filter((c) => c.origen === 'vivofacil');
  const completadas = citas.filter((c) => c.estado_cita === 'completada');
  const pendientesValidar = citas.filter((c) => c.estado_cita !== 'completada' && c.estado_cita !== 'cancelada');
  const total = completadas.length * VIVOFACIL_VALOR_SESION;

  const porPaciente = new Map<string, number>();
  completadas.forEach((c) => porPaciente.set(c.cliente_nombre, (porPaciente.get(c.cliente_nombre) ?? 0) + 1));
    ...
    >
      ...
      <div className="glass-panel overflow-x-auto rounded-2xl">
        <table className="w-full min-w-[480px] text-left text-xs">
          ...
          <tbody>
            {pacientes.length === 0 ? (
              <tr><td colSpan={3} className="px-4 py-6 text-center text-zinc-600">Sin sesiones Vivofácil este mes</td></tr>
            ) : (
              pacientes.map((p) => (
                <tr key={p.nombre} className="border-b border-white/5 last:border-0 hover:bg-white/[0.03]">
