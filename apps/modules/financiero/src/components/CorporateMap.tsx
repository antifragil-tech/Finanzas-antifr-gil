...
function buildLayout(sociedades: DashboardData['sociedades']): {
  ...
  const pavierOnly = directSubs.filter(
    (s) => normP(s['% Pavier'] as number) > 0 && normP(s['% Armia'] as number) === 0,
  );
  const armiaOnly = directSubs.filter(
    (s) => normP(s['% Armia'] as number) > 0 && normP(s['% Pavier'] as number) === 0,
  );
  const shared = directSubs.filter(
    (s) => normP(s['% Pavier'] as number) > 0 && normP(s['% Armia'] as number) > 0,
  );
  ...
  orderedL2.forEach((soc, i) => {
    ...
    nodes.push({
      ...
      nombre,
      level: 2,
      x: centerX(i, orderedL2.length),
      y: LEVEL_Y[2],
      unconfigured: noConfig,
      pctLabel,
    });
  ...
  const pavierX =
    pavierChildren.length > 0 ? Math.min(...pavierChildren.map((n) => n.x)) : centerX(0, 2);
  const armiaX =
    armiaChildren.length > 0 ? Math.max(...armiaChildren.map((n) => n.x)) : centerX(1, 2);
  ...
  const indirectSubs = socs.filter(
    (s) => !!s.parent_sociedad_id && !isPersona(s['ID-Ref'] as string),
  );
  ...
  return (
    ...
    >
      <rect
        ...
        className={isPerson ? '' : 'transition-all duration-150 hover:brightness-125'}
      />
      <text
        x={node.x}
        y={nameY}
      ...
      {warn && (
        <g transform={`translate(${node.x + nodeW / 2 - 10}, ${node.y - 8})`}>
          ...
          <text textAnchor="middle" y={4} fill="white" fontSize={10} fontWeight="900">
            !
          </text>
        </g>
      )}
...
export function CorporateMap({ data, onSelectSociety, onClose }: Props) {
  ...
  const l2WithChildren = nodes.filter(
    (n) => n.level === 2 && (edgesByParent.get(n.id)?.length ?? 0) > 0,
  );

  return (
    <div className="relative h-full min-h-[700px] w-full">
      ...
      {isEmpty ? (
        <div className="flex flex-col items-center justify-center space-y-3 py-32 text-zinc-600">
          <p className="text-sm font-medium uppercase tracking-widest">
            Sin estructura societaria configurada
          </p>
          <p className="text-xs text-zinc-700">
            Configura las sociedades y sus % de participación en Datos Maestros
          </p>
        </div>
      ) : (
        <div className="w-full overflow-auto rounded-2xl border border-white/[0.06] bg-zinc-900/30">
          <svg width={W} height={svgH} viewBox={`0 0 ${W} ${svgH}`} className="mx-auto block">
            ...
            <g transform={`translate(20, ${svgH - 44})`}>
              <rect
                ...
                strokeWidth={1}
              />
              <text x={18} y={9} fill="#52525b" fontSize={9}>
                Propietario
              </text>
              <rect
                x={120}
                ...
                strokeWidth={1.5}
              />
              <text x={134} y={8} fill="#52525b" fontSize={9}>
                Holding
              </text>
              <rect
                x={200}
                ...
                strokeWidth={1}
              />
              <text x={214} y={8} fill="#52525b" fontSize={9}>
                Filial
              </text>
            </g>
          </svg>
