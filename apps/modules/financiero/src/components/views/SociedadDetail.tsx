...
const EMPTY_KPIS = (idRef: string, nombre: string): KpisSocRow => ({
  ...
  nombre,
  tipo: 'Sociedad',
  caja_disponible: null,
  deuda_bancaria_cp: null,
  deuda_bancaria_lp: null,
  deuda_bancaria: null,
  deuda_socios: null,
  deuda_financiera_neta: null,
...
export function SociedadDetail({
  sociedad,
  onBack,
  onRefresh,
  onNavigateToPatrimonio,
  backLabel = 'Volver a Datos Maestros',
}: Props) {
    ...
    <div className="max-w-4xl space-y-8 p-8">
      ...
      <div>
        ...
        <div className="flex items-center gap-4">
          <div className="rounded-2xl border border-blue-500/20 bg-blue-500/15 p-3 text-blue-400">
            <Building2 size={22} />
          </div>
          <div>
            <h1 className="text-2xl font-semibold tracking-tight text-white">{sociedad.nombre}</h1>
            <p className="mt-0.5 font-mono text-xs text-zinc-500">
              {sociedad.id_ref}
              {sociedad.cif ? ` · ${sociedad.cif}` : ''}
            </p>
          </div>
        </div>
      </div>
      ...
      <Card icon={<Building2 size={15} />} title="Datos Básicos">
        <div className="grid grid-cols-2 gap-4">
          <Field
            label="Nombre"
            value={info.nombre}
            onChange={(v) => setInfo((f) => ({ ...f, nombre: v }))}
          />
          <Field
            label="CIF"
            value={info.cif ?? ''}
            onChange={(v) => setInfo((f) => ({ ...f, cif: v || null }))}
            placeholder="B-00000000"
          />
          <Field
            label="Holding Principal"
            value={info.holding_principal ?? ''}
            onChange={(v) => setInfo((f) => ({ ...f, holding_principal: v || null }))}
          />
          <div>
            ...
            <select
              ...
              className="mt-1 w-full rounded-lg border border-white/10 bg-zinc-900 px-3 py-2 text-sm text-white transition-colors focus:border-blue-500/50 focus:outline-none"
            >
          ...
          <Field
            label="% Pavier (0–1)"
            value={String(info.pct_pavier ?? '')}
            onChange={(v) => setInfo((f) => ({ ...f, pct_pavier: v ? parseFloat(v) : null }))}
            type="number"
            placeholder="0.5"
          />
          <Field
            label="% Armia (0–1)"
            value={String(info.pct_armia ?? '')}
            onChange={(v) => setInfo((f) => ({ ...f, pct_armia: v ? parseFloat(v) : null }))}
            type="number"
            placeholder="0.5"
          />
          <div className="col-span-2 mt-1 border-t border-white/5 pt-4">
            <p className="text-2xs mb-3 font-semibold uppercase tracking-widest text-zinc-600">
              Participación intermedia (si no es filial directa del holding)
            </p>
            <div className="grid grid-cols-2 gap-4">
              <Field
                label="Sociedad Matriz (ID-Ref)"
                value={info.parent_sociedad_id ?? ''}
                onChange={(v) => setInfo((f) => ({ ...f, parent_sociedad_id: v || null }))}
                placeholder="ej: S-PERISUR"
              />
              <Field
                label="% Propiedad de la Matriz (0–1)"
                value={String(info.pct_en_sociedad_padre ?? '')}
                onChange={(v) =>
                  setInfo((f) => ({ ...f, pct_en_sociedad_padre: v ? parseFloat(v) : null }))
                }
                type="number"
                placeholder="0.5"
              />
        ...
        {loadingKpis ? (
          ...
          <>
            <p className="text-2xs mb-4 font-medium uppercase tracking-widest text-zinc-600">
              Posición de liquidez y deuda
            </p>
            <div className="grid grid-cols-2 gap-4">
              <NumField
                label="Caja Disponible (€)"
                value={kpis.caja_disponible}
                onChange={(v) => setKpis((k) => ({ ...k, caja_disponible: v }))}
              />
              <NumField
                label="Deuda de Socios (€)"
                value={kpis.deuda_socios}
                onChange={(v) => setKpis((k) => ({ ...k, deuda_socios: v }))}
              />
              <NumField
                label="Deuda Bancaria C/P (€)"
                value={kpis.deuda_bancaria_cp}
                onChange={(v) => setKpis((k) => ({ ...k, deuda_bancaria_cp: v }))}
              />
              <NumField
                label="Deuda Bancaria L/P (€)"
                value={kpis.deuda_bancaria_lp}
                onChange={(v) => setKpis((k) => ({ ...k, deuda_bancaria_lp: v }))}
              />
            </div>
            <div className="mb-4 mt-5 flex items-center gap-2">
              <p className="text-2xs font-medium uppercase tracking-widest text-zinc-600">
                Balance simplificado
              </p>
              <span className="text-2xs flex items-center gap-1 rounded-full border border-white/5 bg-zinc-800 px-2 py-0.5 font-medium uppercase tracking-wider text-zinc-500">
                <BookOpen size={9} />
                Desde libro mayor
              </span>
            </div>
            <div className="grid grid-cols-2 gap-4">
              ...
              <ReadField
                ...
                value={null}
                dateValue={kpis.fecha_actualizacion}
              />
            </div>
      ...
      <Card icon={<History size={15} />} title="Histórico de KPIs">
        <p className="mb-5 text-xs text-zinc-500">
          Evolución de la sociedad período a período, derivada de los balances de sumas y saldos
          importados.
        </p>
          ...
          <div className="flex flex-col items-center gap-3 py-8 text-zinc-600">
            ...
            {onNavigateToPatrimonio && (
              <button
                ...
                className="flex items-center gap-2 rounded-xl border border-emerald-500/20 bg-emerald-500/10 px-4 py-2 text-xs font-medium text-emerald-400 transition-all hover:bg-emerald-500/20"
              >
          ...
          <div className="space-y-3">
              ...
              <div className="rounded-xl border border-white/5 bg-white/[0.02] p-3 text-center">
                ...
                <p className="mt-1 text-lg font-semibold text-emerald-400">
                  {formatCurrency(
                    activos.reduce(
                      (s, a) =>
                        s + (a.valor_actual * a.porcentaje_propiedad) / 100 - (a.deuda_viva ?? 0),
                      0,
                    ),
                  )}
                </p>
            ...
            <div className="space-y-2">
              {activos.map((a) => (
                <div
                  key={a.id}
                  className="flex items-center justify-between rounded-xl border border-white/[0.04] bg-white/[0.02] px-4 py-3 transition-all hover:border-white/10"
                >
                  <div>
                    <span className="text-sm font-medium text-white">{a.nombre}</span>
                    {a.subcategoria && (
                      <span className="text-2xs ml-2 font-semibold uppercase tracking-widest text-zinc-500">
                        {a.subcategoria}
                      </span>
                    )}
                  </div>
            ...
            {onNavigateToPatrimonio && (
              <button
                onClick={onNavigateToPatrimonio}
                className="mt-2 flex items-center gap-2 rounded-xl border border-emerald-500/20 bg-emerald-500/10 px-4 py-2 text-xs font-medium text-emerald-400 transition-all hover:bg-emerald-500/20"
              >
      ...
      <Card icon={<BookOpen size={15} />} title="Libro de Contabilidad — Balance PGC">
        <p className="mb-4 text-xs text-zinc-500">
          Sube el libro mayor de esta sociedad para calcular el balance PGC completo y guardarlo en
          Supabase.
        </p>
        <BalanceView
  ...
  return (
    <div>
      ...
      <input
        ...
        className="mt-1 w-full rounded-lg border border-white/10 bg-zinc-900 px-3 py-2 text-sm text-white transition-colors placeholder:text-zinc-600 focus:border-blue-500/50 focus:outline-none"
      />
    </div>
  );
}
  ...
  return (
    <div className="mt-5 flex justify-end">
      <button
        onClick={onClick}
        disabled={saving}
        className={`flex items-center gap-2 rounded-xl px-5 py-2.5 text-sm font-medium transition-all disabled:opacity-50 ${saved ? 'border border-emerald-500/30 bg-emerald-500/20 text-emerald-300' : 'border border-blue-500/30 bg-blue-500/20 text-blue-200 hover:bg-blue-500/30'}`}
      >
        {saving ? (
...
function ReadField({
  ...
  return (
    <div>
      <label className="text-2xs block font-medium uppercase tracking-widest text-zinc-600">
        {label}
      </label>
      <div className="mt-1 w-full select-none rounded-lg border border-white/[0.04] bg-zinc-900/40 px-3 py-2 text-sm text-zinc-500">
        {display}
      </div>
    </div>
  );
