...
export function HistoricoKpisSection({ societyIdRef }: Props) {
  ...
  if (error) {
    return <p className="py-2 text-xs text-rose-400">{error}</p>;
  }
    ...
    return (
      <div className="flex flex-col items-center gap-2 py-8 text-zinc-600">
        ...
        <p className="text-2xs text-zinc-700">
          Sube balances de sumas y saldos mensuales para construir el histórico
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {prev && (
        <p className="text-2xs -mt-3 text-zinc-600">Variación vs {fmtPeriodo(prev.periodo)}</p>
      )}
          ...
          <div className="mb-3 flex flex-wrap items-center gap-1.5">
            {CHART_METRICS.map((m) => (
              <button
                ...
                className={`text-2xs rounded-full border px-3 py-1 font-medium uppercase tracking-wider transition-colors ${
                  chartKey === m.key
              ...
              <ComposedChart data={chartData} margin={{ top: 8, right: 8, bottom: 0, left: 8 }}>
                ...
                <Tooltip
                  ...
                  formatter={(value: number) => [
                    ...
                    CHART_METRICS.find((m) => m.key === chartKey)?.label ?? '',
                  ]}
                />
                <Area
                  dataKey="valor"
...
function KpiCell({
  ...
  const deltaPct =
    delta !== null && prevValue !== undefined && Math.abs(prevValue) > 0.005
  ...
  return (
    <div className="rounded-xl border border-white/5 bg-white/[0.02] p-3">
      <p className="text-2xs font-medium uppercase tracking-widest text-zinc-500">{label}</p>
      <p className="mt-1 text-lg font-light tracking-tight text-zinc-100">
        {formatCurrency(value)}
      </p>
      {delta !== null &&
        (isFlat ? (
          <p className="text-2xs mt-0.5 text-zinc-600">sin variación</p>
        ) : (
          <p
            className={`text-2xs mt-0.5 font-medium ${good ? 'text-emerald-500' : 'text-rose-400'}`}
          >
