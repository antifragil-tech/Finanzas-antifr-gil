        ...
        {sorted.map(({ nombre, valor, pct }) => (
          <div key={nombre} className="flex items-center justify-between gap-4">
            <div className="flex min-w-0 items-center gap-1.5">
              <span className="text-2xs shrink-0 font-medium text-zinc-600">
                {(pct * 100).toFixed(0)}%
              </span>
              <span className="truncate text-xs text-zinc-300">{nombre}</span>
            </div>
            <span className="shrink-0 text-xs font-semibold text-white">{mask(valor)}</span>
          </div>
        ))}
      </div>
    </div>
...
function mulRow(r: KpiRow, w: number): KpiRow {
  return {
    caja: r.caja * w,
    deudaBancaria: r.deudaBancaria * w,
    deudaBancariaLP: r.deudaBancariaLP * w,
    deudaBancariaCP: r.deudaBancariaCP * w,
    deudaSocios: r.deudaSocios * w,
    fondoManiobra: r.fondoManiobra * w,
    activoCorriente: r.activoCorriente * w,
    pasivoCorriente: r.pasivoCorriente * w,
    patrimonioNeto: r.patrimonioNeto * w,
    activoTotal: r.activoTotal * w,
    pasivoTotal: r.pasivoTotal * w,
  };
}
...
function flattenSociety(
  ...
  if (fin)
    result.push({ idRef: societyId, nombre, pct: effectivePct, ...parseRow(fin, effectivePct) });
  ...
  if (holdingCol) {
    ...
    for (const child of sociedades ?? []) {
      ...
      result.push(
        ...flattenSociety(child['ID-Ref'], finanzas, sociedades, effectivePct * childPct, next),
      );
    }
  } else {
...
function effectiveGroupPct(
  ...
  if (soc.parent_sociedad_id) {
    return (
      effectiveGroupPct(soc.parent_sociedad_id, sociedades, visited) *
...
export function Overview({ data, selectedSociety, isPrivateMode }: OverviewProps) {
  const kpis = useMemo(() => {
    ...
    const pavierSoc = data.sociedades?.find((s) =>
      (s['Nombre de la Sociedad'] as string)?.toLowerCase().includes('pavier'),
    );
    const armiaSoc = data.sociedades?.find((s) =>
      (s['Nombre de la Sociedad'] as string)?.toLowerCase().includes('armia'),
    );
    ...
    if (selectedSociety.idRef === null) {
      ...
      flatItems = data.finanzas_sociedades.map((soc) => {
        ...
        const nombre =
          (socMeta?.['Nombre de la Sociedad'] as string) ?? (soc.Nombre as string) ?? idRef;
  ...
  const [periodById, setPeriodById] = useState<{
    periodo: string;
    map: Map<string, KpiRow>;
  } | null>(null);
  ...
  useEffect(() => {
    ...
    void Promise.all(
      ...
      .finally(() => {
        if (!cancelled) setLoadingPeriodo(false);
      });
    return () => {
      cancelled = true;
    };
  ...
  const view = useMemo(() => {
    ...
    const missing = kpis.flatItems
      .filter((it) => !periodById.map.has(it.idRef))
      .map((it) => it.nombre);
    return {
      propio,
      consolidated,
      isHolding: kpis.isHolding,
      hasChildren: kpis.hasChildren,
      breakdown: kpis.hasChildren ? items : null,
      unconfigured: kpis.unconfigured,
      flatItems: items,
      periodoActivo: selPeriodo,
      missing,
    };
      ...
      <div className="space-y-12">
        ...
        <div className="py-12 text-center text-zinc-600">
          ...
          <p className="mt-1 text-xs">
            Sube el libro mayor desde Datos Maestros para ver el balance.
          </p>
        </div>
      </div>
    );
  ...
  const { propio, consolidated, hasChildren, breakdown, unconfigured, periodoActivo, missing } =
    view ?? { ...kpis, periodoActivo: null, missing: [] };

  // Índice del selector: 0..n-1 = períodos (asc). El último es el más reciente.
  const selIdx =
    selPeriodo === null ? periodos.length - 1 : Math.max(0, periodos.indexOf(selPeriodo));
  const setIdx = (i: number) => {
    const p = periodos[i];
    if (p) setSelPeriodo(p);
  };

  const bdTooltip = (
    label: string,
    field: 'caja' | 'deudaBancaria' | 'deudaSocios' | 'fondoManiobra' | 'patrimonioNeto',
  ) =>
    breakdown ? (
      <BreakdownTooltip
        label={label}
        items={breakdown.map((b) => ({ nombre: b.nombre, valor: b[field], pct: b.pct }))}
        mask={mask}
      />
    ) : undefined;
    ...
    <div className="space-y-12">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="space-y-2">
          ...
          <p className="text-xs font-medium text-zinc-500">
            {periodoActivo ? (
              <>
                Balance a <span className="text-blue-300">{fmtPeriodo(periodoActivo)}</span> —
                derivado del sumas y saldos del período
              </>
            ) : (
              <>
                Posición de liquidez y deuda a valor contable ·{' '}
                {hasChildren
                  ? `${selectedSociety.nombre} — consolidado ponderado`
        ...
        {selectedSociety.idRef !== null && periodos.length > 0 && (
          <div className="flex items-center gap-1">
            <button
              ...
              disabled={selIdx === 0}
              className="rounded-lg p-1.5 text-zinc-400 transition-colors hover:bg-white/5 hover:text-zinc-100 disabled:opacity-30 disabled:hover:bg-transparent"
            ...
            <select
              ...
              className="rounded-lg border border-white/10 bg-zinc-900 px-3 py-1.5 text-sm text-white transition-colors focus:border-blue-500/50 focus:outline-none"
            >
              {periodos.map((p, i) => (
            ...
            {loadingPeriodo && <RefreshCw size={13} className="ml-1 animate-spin text-zinc-500" />}
          </div>
        )}
      </div>
        ...
        <div className="flex items-start gap-3 rounded-xl border border-amber-500/25 bg-amber-500/10 px-4 py-3.5 text-amber-300">
          ...
          <p className="text-xs leading-snug">
            <span className="font-semibold uppercase tracking-widest">
              Sin balance de {fmtPeriodo(periodoActivo)}:
            </span>{' '}
            {missing.join(', ')} — sus importes constan a 0 en esta vista. Sube su sumas y saldos
            del período para completar el consolidado.
          </p>
        </div>
      )}
        ...
        <div className="flex items-start gap-3 rounded-xl border border-amber-500/25 bg-amber-500/10 px-4 py-3.5 text-amber-300">
          ...
          <div>
            <p className="mb-0.5 text-xs font-semibold uppercase tracking-widest">
              % de participación sin configurar
            </p>
            <p className="text-xs leading-snug text-amber-400/80">
              {unconfigured.join(', ')} — se están contabilizando al 100% en vez del % real del
              grupo. Entra en{' '}
              <span className="font-medium text-amber-300">Datos Maestros → Sociedades</span> y
              configura los campos <span className="font-mono">% Pavier</span> y{' '}
              <span className="font-mono">% Armia</span> para corregirlo.
            </p>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 gap-8 md:grid-cols-2 xl:grid-cols-3">
        <KPICard
          ...
          color="emerald"
          tooltip={
            bdTooltip('Caja disponible', 'caja') ??
            'Saldos bancarios y liquidez inmediata operativa.'
          }
        />
        ...
        <KPICard
          ...
          color="amber"
          tooltip={
            bdTooltip('Deuda socios', 'deudaSocios') ??
            'Préstamos de socios y cuentas corrientes con socios.'
          }
        />
        ...
        <KPICard
          ...
          color="blue"
          tooltip={
            bdTooltip('Fondo de maniobra', 'fondoManiobra') ?? (
              <div className="space-y-2 text-left">
                <div className="flex items-center justify-between gap-4">
                  <span className="text-zinc-400">Activo Corriente:</span>
                  <span className="font-semibold text-white">
                    {mask(consolidated.activoCorriente)}
                  </span>
                </div>
                <div className="flex items-center justify-between gap-4">
                  <span className="text-zinc-400">Pasivo Corriente:</span>
                  <span className="font-semibold text-white">
                    {mask(consolidated.pasivoCorriente)}
                  </span>
                </div>
              </div>
            )
        ...
        <KPICard
          ...
          color="blue"
          tooltip={
            bdTooltip('Patrimonio neto', 'patrimonioNeto') ?? (
              <div className="space-y-2 text-left">
                <div className="flex items-center justify-between gap-4">
                  <span className="text-zinc-400">Activo Total:</span>
                  <span className="font-semibold text-white">{mask(consolidated.activoTotal)}</span>
                </div>
                <div className="flex items-center justify-between gap-4">
                  <span className="text-zinc-400">Pasivo Total:</span>
                  <span className="font-semibold text-white">{mask(consolidated.pasivoTotal)}</span>
                </div>
              </div>
            )
          }
