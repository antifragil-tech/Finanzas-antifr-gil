...
import { Building2, Globe, Crown, ChevronRight, Wallet, Briefcase, Pencil, RefreshCw } from 'lucide-react';
...
function effectiveGroupPct(
  ...
  if (soc.parent_sociedad_id) {
    return effectiveGroupPct(soc.parent_sociedad_id as string, sociedades, visited)
...
function KpiCell({ label, value, color, breakdown, field, isPrivateMode }: {
  label: string;
  value: string;
  color: string;
  breakdown?: BreakdownItem[];
  ...
  return (
    <div className="group/kpi relative">
      <p className="text-2xs text-zinc-600 font-semibold uppercase tracking-widest">{label}</p>
      <p className={`text-sm font-semibold mt-0.5 ${color} ${isPrivateMode ? 'blur-sm' : ''}`}>{value}</p>
      {sorted && sorted.length > 1 && !isPrivateMode && (
        <div className="absolute bottom-full left-0 mb-2 w-56 bg-zinc-900 border border-white/[0.1] rounded-xl p-2.5 z-50 pointer-events-none shadow-xl opacity-0 group-hover/kpi:opacity-100 transition-opacity duration-150 text-left">
          <p className="text-2xs font-semibold text-zinc-500 uppercase tracking-widest mb-2">{label} por sociedad</p>
          <div className="space-y-1">
            {sorted.map(b => (
              <div key={b.nombre} className="flex items-center justify-between gap-2">
                <span className="text-2xs text-zinc-400 truncate">{b.nombre}</span>
                <span className={`text-2xs font-medium shrink-0 ${field === 'deuda' ? 'text-rose-400' : b[field] >= 0 ? 'text-zinc-200' : 'text-rose-400'}`}>
                  {formatCurrency(b[field])}
                </span>
              </div>
            ))}
          </div>
          <div className="absolute top-full left-4 w-2 h-2 bg-zinc-900 border-r border-b border-white/[0.1] rotate-45 -mt-1" />
        </div>
      )}
    </div>
  );
...
function EntityCard({ society, kpis, breakdown, projectCount, isPrivateMode, onClick, onOpenFicha }: CardProps) {
  ...
  const accent = isPersona
    ? { border: 'border-amber-500/20 hover:border-amber-400/40', icon: 'bg-amber-500/20 text-amber-400', arrow: 'group-hover:text-amber-400' }
    : isGlobal
    ? { border: 'border-blue-500/20 hover:border-blue-400/40', icon: 'bg-blue-500/20 text-blue-400', arrow: 'group-hover:text-blue-400' }
    : isHolding
    ? { border: 'border-blue-500/15 hover:border-blue-400/30', icon: 'bg-blue-500/15 text-blue-400', arrow: 'group-hover:text-blue-400' }
    : { border: 'border-white/[0.07] hover:border-white/20', icon: 'bg-zinc-800 text-zinc-400', arrow: 'group-hover:text-zinc-300' };
  ...
  return (
    <div
      ...
      className={`w-full text-left rounded-2xl border bg-white/[0.02] p-5 transition-all group hover:bg-white/[0.04] cursor-pointer ${accent.border}`}
    >
      <div className="flex items-start justify-between gap-2">
        <div className="flex items-center gap-3 min-w-0">
          <div className={`p-2.5 rounded-xl shrink-0 ${accent.icon}`}>
            <Icon size={16} />
          </div>
          <div className="min-w-0">
            <p className="text-sm font-semibold text-white leading-tight truncate">{society.nombre}</p>
            {society.badge && <p className="text-2xs font-semibold uppercase tracking-widest text-zinc-600 mt-0.5">{society.badge}</p>}
            {society.cif && <p className="text-2xs text-zinc-700 mt-0.5">CIF {society.cif}</p>}
          </div>
        </div>
        <div className="flex items-center gap-1 shrink-0 mt-0.5">
          {onOpenFicha && (
            <button
              ...
              className="p-1.5 rounded-lg text-zinc-700 hover:text-blue-400 hover:bg-blue-500/10 transition-colors"
            >
              <Pencil size={13} />
            </button>
          )}
      ...
      {isPersona ? (
        <div className="mt-4 pt-3 border-t border-white/[0.05] flex items-center gap-2 text-zinc-600">
          <Wallet size={11} />
          <span className="text-2xs font-semibold uppercase tracking-widest">Patrimonio personal</span>
        </div>
      ) : kpis ? (
        <div className="mt-4 grid grid-cols-3 gap-2">
          <KpiCell label="Caja"  value={v(kpis.caja)}  color="text-white"
            field="caja"  isPrivateMode={isPrivateMode} {...(breakdown ? { breakdown } : {})} />
          <KpiCell label="Deuda" value={v(kpis.deuda)} color="text-rose-400"
            field="deuda" isPrivateMode={isPrivateMode} {...(breakdown ? { breakdown } : {})} />
          <KpiCell label="Neto"  value={v(kpis.neto)}  color={kpis.neto >= 0 ? 'text-emerald-400' : 'text-rose-400'}
            field="neto"  isPrivateMode={isPrivateMode} {...(breakdown ? { breakdown } : {})} />
        </div>
      ) : (
      ...
      {projectCount > 0 && (
        <div className="mt-3 pt-3 border-t border-white/[0.05] flex items-center gap-2 text-zinc-600">
          <Briefcase size={11} />
          <span className="text-2xs font-semibold uppercase tracking-widest">{projectCount} proyecto{projectCount !== 1 ? 's' : ''}</span>
        </div>
      )}
    </div>
  );
...
export function EntidadesView({ data, availableSocieties, isPrivateMode, onSelectSociety, onRefresh, onNavigateToPatrimonio }: Props) {
  ...
  const consolidatedBreakdown = useMemo<BreakdownItem[]>(() => {
    ...
    return data.finanzas_sociedades
      .map(f => {
        ...
        const resolvedId = (socMeta?.['ID-Ref'] as string) ?? idRef;
        const pct    = effectiveGroupPct(resolvedId, data.sociedades);
        const nombre = (socMeta?.['Nombre de la Sociedad'] as string)
          ?? availableSocieties.find(s => s.idRef === resolvedId)?.nombre
          ?? (f.Nombre as string) ?? idRef;
        const caja  = parseCurrency(f['Caja Disponible']) * pct;
        const deuda = parseCurrency(f['Deuda Bancaria'])  * pct;
  ...
  const personas  = availableSocieties.filter(s => s.id === 'javier_alarcon' || s.id === 'ivan_alarcon');
  ...
  const renderCard = (s: SocietyOption) => {
    ...
    return (
      <EntityCard
        ...
        {...(fichaRef ? { onOpenFicha: () => { void openFicha(fichaRef); } } : {})}
      />
    );
  };
    ...
    <div className="space-y-10">
      <div>
        ...
        <p className="text-xs text-zinc-500 mt-1">
          Selecciona una entidad para ver su detalle, proyectos y balance · el lápiz abre la ficha de gestión
        </p>
        {loadingFicha && (
          <p className="flex items-center gap-2 text-xs text-zinc-500 mt-2">
            <RefreshCw size={12} className="animate-spin" />Abriendo ficha...
          </p>
        )}
        {errorFicha && <p className="text-xs text-rose-400 mt-2">{errorFicha}</p>}
      </div>

      {personas.length > 0 && (
        <div className="space-y-3">
          <p className="text-2xs font-semibold text-zinc-500 uppercase tracking-widest">Personas</p>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">{personas.map(renderCard)}</div>
        </div>
      )}

      {global.length > 0 && (
        <div className="space-y-3">
          <p className="text-2xs font-semibold text-zinc-500 uppercase tracking-widest">Consolidado</p>
      ...
      {holdings.length > 0 && (
        <div className="space-y-3">
          <p className="text-2xs font-semibold text-zinc-500 uppercase tracking-widest">Holdings</p>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">{holdings.map(renderCard)}</div>
        </div>
      )}

      {filiales.length > 0 && (
        <div className="space-y-3">
          <p className="text-2xs font-semibold text-zinc-500 uppercase tracking-widest">Filiales</p>
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">{filiales.map(renderCard)}</div>
        </div>
      )}
    </div>
  );
