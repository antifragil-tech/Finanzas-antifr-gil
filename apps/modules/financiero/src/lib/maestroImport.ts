...
export async function updateSociedad(idRef: string, data: Partial<Omit<SociedadRow, 'id_ref'>>): Promise<void> {
...
export async function updateProyecto(idRef: string, data: Partial<Omit<ProyectoRow, 'id_ref'>>): Promise<void> {
...
export async function saveKpisSociedad(data: KpisSocRow): Promise<void> {
  ...
  }).then(async r => { if (!r.ok) throw new Error(`saveKpis: ${r.status} — ${(await r.text()).slice(0, 200)}`); });
}
...
export async function saveKpisProyecto(data: KpisProjRow): Promise<void> {
  await fetch(sbUrl('kpis_proyectos'), {
    ...
    headers: h({ Prefer: 'resolution=merge-duplicates' }),
    body: JSON.stringify(data),
  }).then(async r => { if (!r.ok) throw new Error(`saveKpisProj: ${r.status} — ${(await r.text()).slice(0, 200)}`); });
}
...
export async function getProyectoDetalle(idRef: string): Promise<ProyectoDetalle | null> {
  const r = await req('GET', `proyectos?id_ref=eq.${encodeURIComponent(idRef)}&select=id_ref,nombre,sociedad_tenedora,estado,fecha_inicio,fecha_prevista_salida,fecha_salida_real,tipo_activo,ubicacion,superficie_m2&limit=1`);
  const d = await r.json() as ProyectoDetalle[];
  return d.length ? (d[0] ?? null) : null;
}

export async function updateProyectoDetalle(idRef: string, data: Partial<Omit<ProyectoDetalle, 'id_ref'>>): Promise<void> {
  await req('PATCH', `proyectos?id_ref=eq.${encodeURIComponent(idRef)}`, data);
}
...
export async function getFlujosCajaProyecto(proyectoId: string): Promise<FlujoCajaRow[]> {
  const r = await req('GET', `flujos_caja_proyectos?proyecto_id=eq.${encodeURIComponent(proyectoId)}&order=fecha`);
  return r.json() as Promise<FlujoCajaRow[]>;
}

export async function insertFlujoCaja(data: Omit<FlujoCajaRow, 'id' | 'created_at' | 'updated_at'>): Promise<FlujoCajaRow> {
  const res = await fetch(sbUrl('flujos_caja_proyectos'), {
    method: 'POST',
    headers: h({ Prefer: 'return=representation' }),
    body: JSON.stringify(data),
  });
  if (!res.ok) throw new Error(`insertFlujoCaja: ${res.status} — ${(await res.text()).slice(0, 200)}`);
  const rows = await res.json() as FlujoCajaRow[];
  if (!rows[0]) throw new Error('insertFlujoCaja: respuesta vacía de Supabase');
  return rows[0];
}

export async function updateFlujoCaja(id: string, data: Partial<Omit<FlujoCajaRow, 'id' | 'proyecto_id'>>): Promise<void> {
