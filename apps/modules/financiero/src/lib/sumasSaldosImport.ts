...
export function fmtPeriodoCorto(iso: string): string {
  if (iso === PERIODO_INICIAL) return 'Inicial';
  return new Date(iso + 'T00:00:00').toLocaleDateString('es-ES', { month: 'short', year: '2-digit' });
}

// ── Queries ───────────────────────────────────────────────────────────────────

export async function getPeriodosImportados(): Promise<PeriodoImportado[]> {
  const res = await req('GET', 'v_balance_periodos?select=sociedad_id,periodo,lineas,importado_en&order=periodo.desc');
  const rows = await res.json() as { sociedad_id: string; periodo: string; lineas: number; importado_en: string }[];
  return rows;
}
...
export async function uploadSumasSaldos(
  ...
  await req('DELETE', `balance_sumas_saldos?sociedad_id=eq.${encodeURIComponent(sociedadId)}&periodo=eq.${periodo}`);
...
export async function getPeriodosBySociedad(sociedadId: string): Promise<PeriodoImportado[]> {
  ...
  for (const r of rows) {
    ...
    if (cur) { cur.count++; }
    else { map.set(r.periodo, { count: 1, latest: r.created_at }); }
  }
...
export async function getLineasBSS(sociedadId: string, periodo: string): Promise<BalanceSumasSaldosRow[]> {
  const res = await req(
    'GET',
