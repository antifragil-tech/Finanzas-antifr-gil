...
import {
  CheckSquare, AlertTriangle, Plus, X, Check, Calendar,
  ChevronDown, ChevronRight, Trash2, ArrowRight, GripVertical, Pencil,
} from 'lucide-react';
import {
...
const COLUMNAS: { id: TareaColumna; label: string; color: string; bg: string; dot: string }[] = [
  { id: 'todo',     label: 'Por hacer', color: 'text-blue-400',    bg: 'bg-blue-500/5 border-blue-500/15',     dot: 'bg-blue-500' },
  { id: 'doing',    label: 'En curso',  color: 'text-amber-400',   bg: 'bg-amber-500/5 border-amber-500/15',   dot: 'bg-amber-400 animate-pulse' },
  { id: 'on_hold',  label: 'En espera', color: 'text-violet-400',  bg: 'bg-violet-500/5 border-violet-500/15', dot: 'bg-violet-500' },
  { id: 'done',     label: 'Hecho',     color: 'text-emerald-400', bg: 'bg-emerald-500/5 border-emerald-500/15', dot: 'bg-emerald-500' },
];
...
const RECURRENCIA_DIAS: Record<TareaRecurrencia, number> = {
  diaria: 1, semanal: 7, quincenal: 14, mensual: 30, trimestral: 91, anual: 365,
};
...
function isOverdue(t: ProyectoTarea) {
  return t.columna !== 'done' && t.fecha_limite
    && new Date(t.fecha_limite + 'T00:00:00') < new Date();
}

// ── Empty form ────────────────────────────────────────────────────────────────

const EMPTY: { titulo: string; categoria: TareaCategoria; fecha_limite: string; columna: TareaColumna } = {
  titulo: '', categoria: 'general', fecha_limite: '', columna: 'todo',
};
...
function InlineAddForm({ onAdd, onCancel }: {
  ...
  return (
    <div className="p-2 bg-zinc-900/90 border border-white/[0.1] rounded-lg space-y-1.5">
      <input
        autoFocus
        value={titulo}
...
function DraggableTaskCard({ tarea, onDelete, onEdit, ghost = false }: {
  tarea: ProyectoTarea;
  onDelete: (id: string) => void;
  onEdit: (id: string, data: { titulo: string; categoria: TareaCategoria; fecha_limite: string; descripcion: string; recurrencia: TareaRecurrencia | '' }) => Promise<void>;
  ghost?: boolean;
}) {
  ...
  async function saveEdit() {
    ...
    setSaving(true);
    await onEdit(tarea.id, { titulo: editTitulo.trim(), categoria: editCategoria, fecha_limite: editFecha, descripcion: editDesc, recurrencia: editRecurr });
    setSaving(false);
    setMode('view');
  }
  ...
  if (mode === 'edit') {
    return (
      <div className="bg-zinc-900/90 border border-blue-500/30 rounded-lg p-2.5 space-y-1.5">
        <input
          autoFocus
          value={editTitulo}
        ...
        <textarea
          ...
          rows={2}
          className="w-full bg-zinc-800/60 text-2xs text-zinc-400 placeholder-zinc-600 rounded px-1.5 py-1 outline-none border border-white/[0.06] resize-none"
        />
        <div className="flex items-center gap-1.5 flex-wrap">
          <select
            value={editCategoria}
          ...
          <button onClick={() => void saveEdit()} disabled={saving || !editTitulo.trim()} className="ml-auto text-emerald-400 disabled:opacity-40">
            <Check size={12} />
          </button>
          <button onClick={() => setMode('view')} className="text-zinc-600 hover:text-zinc-400"><X size={12} /></button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`group relative bg-zinc-900/80 border rounded-lg p-2.5 transition-colors select-none ${
        ghost      ? 'shadow-2xl ring-1 ring-blue-500/40 opacity-95' :
    ...
    >
      {/* Tooltip descripción */}
      {hasDesc && (
        <div className="absolute bottom-full left-0 mb-1.5 w-56 bg-zinc-800 border border-white/[0.1] rounded-lg px-3 py-2 text-2xs text-zinc-300 leading-relaxed z-50 pointer-events-none shadow-xl opacity-0 group-hover:opacity-100 transition-opacity duration-150">
          {tarea.descripcion}
          <div className="absolute top-full left-3 w-2 h-2 bg-zinc-800 border-r border-b border-white/[0.1] rotate-45 -mt-1" />
        </div>
      )}
      {mode === 'delete' ? (
        <div className="flex items-center gap-1.5">
          <span className="text-2xs text-zinc-500 flex-1">¿Eliminar?</span>
          <button onClick={() => onDelete(tarea.id)} className="text-rose-400 text-2xs font-medium">Sí</button>
          <button onClick={() => setMode('view')} className="text-zinc-600 text-2xs">No</button>
        </div>
      ) : (
        <>
          <div className="flex items-start gap-1.5">
            <span
              {...attributes}
              {...listeners}
              className="cursor-grab active:cursor-grabbing text-zinc-700 hover:text-zinc-500 mt-0.5 shrink-0 touch-none"
            >
              <GripVertical size={11} />
            </span>
            <p className="text-xs text-white font-medium leading-snug flex-1">{tarea.titulo}</p>
          </div>
          <div className="flex items-center gap-1 flex-wrap mt-1.5 ml-4">
            <span className={`text-2xs font-medium uppercase tracking-wider px-1.5 py-0.5 rounded ${catColor(tarea.categoria)}`}>
              {tarea.categoria}
            </span>
            {dateStr && (
            ...
            {tarea.recurrencia && (
              <span className="text-2xs font-medium text-indigo-400 bg-indigo-500/10 px-1.5 py-0.5 rounded">
            ...
            {!ghost && (
              <div className="ml-auto flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-all">
                <button onClick={openEdit} className="text-zinc-600 hover:text-blue-400 transition-colors">
                  <Pencil size={10} />
                </button>
                <button onClick={() => setMode('delete')} className="text-zinc-600 hover:text-rose-400 transition-colors">
                  <Trash2 size={10} />
                </button>
              </div>
            )}
...
function KanbanColumn({ col, tareas, activeId, onDelete, onEdit, onAdd }: {
    ...
    >
      ...
      <div className="flex items-center justify-between px-1 mb-2 shrink-0">
        ...
        >
          ...
          <span className="text-2xs font-medium text-zinc-600 bg-zinc-800/50 rounded-full px-1.5">{tareas.length}</span>
        ...
        <button onClick={() => setAdding(true)} className={`${col.color} opacity-50 hover:opacity-100 transition-opacity`}>
          <Plus size={11} />
        </button>
      </div>
      ...
      {adding && (
        <div className="mb-1.5">
          <InlineAddForm
            onAdd={async (t, c, f) => { await onAdd(t, c, f); setAdding(false); }}
...
function ProjectKanbanBoard({ proyectoId, tareas, filterCat, onColumnChange, onDelete, onEdit, onAdd }: {
  proyectoId: string;
  tareas: ProyectoTarea[];
  ...
  onDelete: (id: string) => void;
  onEdit: (id: string, data: { titulo: string; categoria: TareaCategoria; fecha_limite: string; descripcion: string; recurrencia: TareaRecurrencia | '' }) => Promise<void>;
  onAdd: (proyectoId: string, col: TareaColumna, titulo: string, cat: TareaCategoria, fecha: string) => Promise<void>;
}) {
  ...
  return (
    ...
    >
      <div className="grid grid-cols-4 gap-2">
        {COLUMNAS.map(col => (
          <KanbanColumn
            ...
            col={col}
            activeId={activeId}
            tareas={filtered.filter(t => t.columna === col.id).sort((a, b) => (a.orden ?? 0) - (b.orden ?? 0))}
            onDelete={onDelete}
            onEdit={onEdit}
      ...
      <DragOverlay dropAnimation={null}>
        {activeTarea ? <DraggableTaskCard tarea={activeTarea} onDelete={() => undefined} onEdit={async () => undefined} ghost /> : null}
      </DragOverlay>
    </DndContext>
  );
}
...
function SortableProject({ id, children }: { id: string; children: (dragHandle: React.ReactNode) => React.ReactNode }) {
  ...
  const dragHandle = (
    <span
      {...attributes}
      {...listeners}
      className="cursor-grab active:cursor-grabbing text-zinc-700 hover:text-zinc-400 transition-colors touch-none p-1 rounded"
    >
...
export function PanelTareas({ proyectos, onGoToProject }: Props) {
  ...
  const allProyectoIds = useMemo(() => {
    const known = new Set([
      ...tareas.map(t => t.proyecto_id),
      ...proyectos.map(p => p.id_ref),
    ]);
  ...
  const overdueCount = useMemo(() =>
    tareas.filter(t => isOverdue(t)).length,
    [tareas]
  );
  ...
  async function handleAdd(
    ...
    const orden = tareas.filter(t => t.proyecto_id === proyectoId && t.columna === columna).length;
  ...
  async function handleEdit(id: string, data: { titulo: string; categoria: TareaCategoria; fecha_limite: string; descripcion: string; recurrencia: TareaRecurrencia | '' }) {
    const prev = tareas.find(t => t.id === id);
    if (!prev) return;
    const recurrencia = (data.recurrencia || null) as TareaRecurrencia | null;
    const updated = { ...prev, titulo: data.titulo, categoria: data.categoria, fecha_limite: data.fecha_limite || null, descripcion: data.descripcion || null, recurrencia };
    setTareas(list => list.map(t => t.id === id ? updated : t));
    const patch: Parameters<typeof updateTarea>[1] = { titulo: data.titulo, categoria: data.categoria, descripcion: data.descripcion };
    ...
    await updateTarea(id, patch)
      .catch(() => setTareas(list => list.map(t => t.id === id ? prev : t)));
  }
  ...
  async function handleNewFormSave() {
    ...
    setSaving(true);
    try {
      await handleAdd(newForm.proyectoId, newForm.columna, newForm.titulo.trim(), newForm.categoria, newForm.fecha_limite);
  ...
  if (loading) {
    return <div className="flex items-center justify-center h-40 text-zinc-600 text-sm">Cargando tareas...</div>;
  }
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      ...
      <div className="flex items-start justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-xl font-semibold text-white tracking-tight">Cuadro de tareas</h1>
          <p className="text-xs text-zinc-500 mt-0.5">
            {activeCount} tareas activas · {proyectos.length} proyectos
            {overdueCount > 0 && <span className="ml-2 text-rose-400 font-medium">{overdueCount} vencidas</span>}
          </p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <select
            ...
            className="bg-zinc-900 border border-white/10 rounded-lg text-2xs text-zinc-400 px-2 py-1.5 outline-none"
          >
          ...
          {/* Nueva tarea */}
          <button
            onClick={() => { setShowNewForm(v => !v); setNewForm({ ...EMPTY, proyectoId: proyectos[0]?.id_ref ?? '' }); }}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-blue-600/15 border border-blue-500/25 text-blue-300 text-xs font-medium hover:bg-blue-600/25 transition-all"
          >
      ...
      {/* ── Alerta vencidas ─────────────────────────────────────────────────── */}
      {overdueCount > 0 && (
        <div className="bg-rose-500/8 border border-rose-500/20 rounded-xl px-4 py-3 flex items-center gap-3">
          <AlertTriangle size={15} className="text-rose-400 shrink-0" />
          <p className="text-sm font-medium text-rose-300">
            {overdueCount} {overdueCount === 1 ? 'tarea vencida' : 'tareas vencidas'} — revisar inmediatamente
          </p>
        </div>
      )}
      ...
      {showNewForm && (
        <div className="bg-zinc-900/60 border border-blue-500/20 rounded-2xl p-5 space-y-4 animate-fade-in">
          <div className="flex items-center justify-between">
            <p className="text-sm font-semibold text-blue-300 uppercase tracking-widest">Nueva tarea</p>
            <button onClick={() => setShowNewForm(false)} className="text-zinc-600 hover:text-zinc-400"><X size={15} /></button>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div className="col-span-2">
              ...
              <input
                ...
                onKeyDown={e => { if (e.key === 'Enter') void handleNewFormSave(); }}
                placeholder="¿Qué hay que hacer?"
                className="w-full mt-1 bg-zinc-900 border border-white/10 rounded-xl px-3 py-2 text-sm text-white focus:outline-none focus:border-blue-500/50"
              />
            </div>
            <div>
              <label className="text-2xs font-semibold text-zinc-500 uppercase tracking-widest">Proyecto *</label>
              <select
              ...
              >
                ...
                {proyectos.map(p => <option key={p.id_ref} value={p.id_ref}>{p.nombre}</option>)}
              </select>
            </div>
            <div>
              <label className="text-2xs font-semibold text-zinc-500 uppercase tracking-widest">Columna</label>
              <select
                value={newForm.columna}
                onChange={e => setNewForm(f => ({ ...f, columna: e.target.value as TareaColumna }))}
                className="w-full mt-1 bg-zinc-900 border border-white/10 rounded-xl px-3 py-2 text-sm text-white focus:outline-none focus:border-blue-500/50"
              >
            ...
            <div>
              <label className="text-2xs font-semibold text-zinc-500 uppercase tracking-widest">Categoría</label>
              <select
                value={newForm.categoria}
                onChange={e => setNewForm(f => ({ ...f, categoria: e.target.value as TareaCategoria }))}
                className="w-full mt-1 bg-zinc-900 border border-white/10 rounded-xl px-3 py-2 text-sm text-white focus:outline-none focus:border-blue-500/50"
              >
                {CATEGORIAS.map(c => <option key={c.id} value={c.id}>{c.label}</option>)}
              </select>
            ...
            <div>
              <label className="text-2xs font-semibold text-zinc-500 uppercase tracking-widest">Fecha límite (opcional)</label>
              <input
                type="date"
          ...
          <div className="flex justify-end gap-2 pt-1">
            <button onClick={() => setShowNewForm(false)} className="px-4 py-2 rounded-xl border border-white/10 text-zinc-400 text-sm font-medium hover:border-white/20 transition-all">
              Cancelar
            </button>
            <button
              onClick={() => void handleNewFormSave()}
              disabled={saving || !newForm.titulo.trim() || !newForm.proyectoId}
              className="flex items-center gap-1.5 px-5 py-2 rounded-xl bg-blue-600/20 border border-blue-500/30 text-blue-300 text-sm font-medium hover:bg-blue-600/30 transition-all disabled:opacity-40"
            >
              {saving ? 'Creando...' : <><Plus size={13} /> Crear tarea</>}
            </button>
          </div>
        </div>
      )}
          ...
          <SortableContext items={allProyectoIds} strategy={verticalListSortingStrategy}>
            <div className="space-y-3">
              {allProyectoIds.map(proyectoId => {
                ...
                const doneCount = tareas.filter(t => t.proyecto_id === proyectoId && t.columna === 'done').length;
                const total     = tareas.filter(t => t.proyecto_id === proyectoId && t.columna !== 'backlog').length;
                  ...
                  <SortableProject key={proyectoId} id={proyectoId}>
                    {dragHandle => (
                      <div className="bg-zinc-900/40 border border-white/[0.06] rounded-2xl overflow-hidden">
                        ...
                        {/* ── Cabecera proyecto ─────────────────────────────── */}
                        <div className="flex items-center gap-1 px-3 py-3.5 hover:bg-white/[0.02] transition-colors">
                          {dragHandle}
                          <button
                          ...
                          >
                            ...
                            <span className="flex-1 text-sm font-semibold text-white truncate">{nombre}</span>
                          </button>
                          <div className="flex items-center gap-2 shrink-0">
                            {overdues > 0 && (
                              <span className="text-2xs font-semibold uppercase tracking-wider px-1.5 py-0.5 rounded bg-rose-500/10 border border-rose-500/20 text-rose-400">
                            ...
                            <span className="text-2xs text-zinc-500 font-semibold">{doneCount}/{total}</span>
                            <button
                              onClick={e => { e.stopPropagation(); onGoToProject(proyectoId); }}
                              className="flex items-center gap-0.5 text-2xs text-zinc-600 hover:text-blue-400 transition-colors uppercase tracking-wider font-medium"
                            >
