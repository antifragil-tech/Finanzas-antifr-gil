...
const NUM_FIELDS: (keyof AnalisisFinanciero)[] = [
  ...
  'presupuesto_obra',
  'gastos_venta',
  'precio_venta_previsto',
  'deuda_promotora',
  'coste_financiero_deuda',
  'precio_compra_escritura',
  'gastos_financiacion_notarial',
  'iva_soportado_obra',
  'precio_venta_reservado',
  'anticipo_comprador',
  'comision_agente_pct',
  'plazo_ejecucion_obra_meses',
  'contingencia_obra_pct',
  'superficie_arrendable_m2',
  'renta_mensual_bruta',
  'gastos_operativos_anuales',
  'capex_inicial',
  'deuda_hipotecaria',
  'cuota_hipoteca_mensual',
  'tasa_ocupacion_prevista_pct',
  'renta_por_m2',
  'cap_rate_salida_pct',
  'ibi_anual',
  'seguro_anual',
  'capex_mantenimiento_anual',
  'retorno_previsto_total',
  'ingresos_anuales_previstos',
  'costes_variables_pct',
  'costes_fijos_anuales',
  'ebitda_objetivo_anual',
  'inversion_circulante',
  ...
  'lgd_pct',
  'valor_garantia',
  'beneficio_esperado_anual',
  'vida_util_activo_anios',
  'valor_residual_pct',
  'tasa_descuento_interna',
  'equity_aportado',
  'horizonte_analisis_anios',
  'valor_residual_estimado',
  'plazo_total_operacion_meses',
  'impuesto_sociedades_pct',
  // Renta — financiación detallada y fiscalidad opcional
  'tipo_interes_deuda_pct',
  'plazo_restante_anios',
  'amortizacion_fiscal_anual',
  'costes_venta_pct',
  'impuesto_plusvalia_pct',
  // Explotación — valor de continuidad / terminal
  'valor_terminal_manual',
  'multiplo_ebitda_salida',
];
...
const STR_FIELDS: (keyof AnalisisFinanciero)[] = [
  ...
  'rating_interno',
  'depreciacion_tipo',
  'metodo_valor_residual',
  'fecha_inicio_explotacion',
  'fecha_inicio_operacion',
  'fecha_salida_prevista',
  'sistema_amortizacion_deuda',
  'metodo_valor_terminal',
  'nivel_riesgo',
  'nivel_liquidez',
];

const BOOL_FIELDS: (keyof AnalisisFinanciero)[] = ['aplicar_fiscalidad', 'aplicar_valor_terminal'];
        ...
        {(Object.keys(TIPO_LABEL) as TipoAnalisis[]).map((t) => {
          ...
          return (
            ...
            >
              ...
              {TIPO_LABEL[t]}
              {isBeta && tipo !== t && (
                <span className="text-2xs ml-1 rounded bg-zinc-800 px-1.5 py-0.5 font-semibold uppercase tracking-widest text-zinc-500">
                  Beta
                </span>
              )}
            </button>
  ...
  useEffect(() => {
    ...
    getAnalisisFinanciero(proyecto.id_ref)
      .then((a) => {
        if (a) {
          setAnalisis(a);
          setTipo(a.tipo_analisis);
          setForm(analisisToForm(a));
        } else {
          setEditing(true);
        }
      })
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex w-fit flex-wrap gap-1 rounded-xl border border-white/[0.06] bg-zinc-900/60 p-1">
          {(
            [
      ...
      {section === 'analisis' && (
        <>
          ...
          <div className="flex items-center justify-between">
            ...
            {analisis && !editing && (
              <button
                ...
                className="flex items-center gap-1.5 rounded-lg border border-white/10 px-3 py-1.5 text-xs font-medium text-zinc-400 transition-all hover:border-white/20 hover:text-white"
              >
          ...
          {analisis && !editing && (
            <>
              {analisis.tipo_analisis === 'compra_venta' && (
                <KpisCV analisis={analisis} proyecto={proyecto} />
              )}
              {analisis.tipo_analisis === 'renta' && (
                <KpisRentaView analisis={analisis} proyecto={proyecto} />
              )}
              {analisis.tipo_analisis === 'explotacion' && (
                <KpisExplotacionView analisis={analisis} proyecto={proyecto} />
              )}
              {analisis.tipo_analisis === 'prestamo' && (
                <KpisPrestamoView analisis={analisis} proyecto={proyecto} />
              )}
              {analisis.tipo_analisis === 'capex_interno' && (
                <KpisCapexView analisis={analisis} proyecto={proyecto} />
              )}
              {analisis.tipo_analisis === 'alternativo' && (
                <KpisAlternativoView analisis={analisis} proyecto={proyecto} />
              )}
              ...
              {analisis.tipo_analisis === 'compra_venta' &&
                ...
                !proyecto.fecha_inicio && (
                  <p className="flex items-center gap-1.5 text-xs text-amber-400">
                    ...
                    Añade fecha de inicio y salida prevista, o plazo total de operación, para
                    calcular la TIR.
                  </p>
                )}
              {/* alternativo: necesita fechas del proyecto */}
              {analisis.tipo_analisis === 'alternativo' &&
                (!proyecto.fecha_inicio || !proyecto.fecha_prevista_salida) && (
                  <p className="flex items-center gap-1.5 text-xs text-amber-400">
                    <AlertTriangle size={12} />
          ...
          {editing && (
            <div className="space-y-6 rounded-2xl border border-white/[0.06] bg-zinc-900/40 p-6">
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  disabled={saving}
                  className="flex items-center gap-2 rounded-xl bg-violet-600 px-5 py-2.5 text-xs font-semibold uppercase tracking-widest text-white transition-colors hover:bg-violet-500 disabled:opacity-50"
                >
      ...
      {section === 'escenarios' && !analisis && (
        <div className="flex flex-col items-center justify-center rounded-2xl border border-white/[0.06] bg-zinc-900/20 py-10">
          <p className="text-sm text-zinc-500">
            Guarda primero un análisis financiero para generar escenarios.
          </p>
        </div>
      )}
      ...
      {section === 'financiacion' && !analisis && (
        <div className="flex flex-col items-center justify-center rounded-2xl border border-white/[0.06] bg-zinc-900/20 py-10">
          <p className="text-sm text-zinc-500">
            Guarda primero un análisis financiero para simular la financiación.
          </p>
        </div>
      )}
