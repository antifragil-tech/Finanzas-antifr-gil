...
import { calcKpisCapexInterno, semaforoTir, semaforoPayback } from '../../../../lib/analisisFinanciero';
...
function DesgloseBlock({ k }: { k: KpisCapexInterno }) {
  ...
  return (
    <Collapsible title="Desglose del CAPEX" subtitle="De la inversión a la caja neta y el valor residual">
      <div className="space-y-0.5">
        <Row label="Inversión CAPEX (año 0)" value={k.inversionCapex != null ? fmt(-k.inversionCapex) : '—'} negativo />
        <Row label="Ahorro / beneficio anual bruto" value={k.ahorroBruto != null ? fmt(k.ahorroBruto) : '—'} />
        {k.capexMantenimiento != null && <Row label="CAPEX de mantenimiento" value={fmt(-k.capexMantenimiento)} isSub negativo />}
        <Row label="Caja neta anual" value={k.cajaNetaAnual != null ? fmt(k.cajaNetaAnual) : '—'} bold />
        <Row label="Vida útil" value={k.vidaUtilUsada != null ? `${k.vidaUtilUsada} años` : '—'} isSub />
        <Row label={`Valor residual${k.residualPorDefecto ? ' (0% por defecto)' : k.residualPct != null ? ` (${k.residualPct}% del coste)` : ''}`} value={k.valorResidual != null ? fmt(k.valorResidual) : '—'} isSub />
      </div>
      <SectionHint>El VAN/TIR usan la caja neta anual (ahorro − mantenimiento) durante la vida útil, más el valor residual sumado al último año. La depreciación es contable y no entra en la caja.</SectionHint>
    </Collapsible>
  );
}
...
function DepreciacionBlock({ analisis, k }: { analisis: AnalisisFinanciero; k: KpisCapexInterno }) {
  return (
    <Collapsible title="Depreciación (dato contable)" subtitle="No afecta a la caja ni al VAN/TIR">
      <KpiGrid cols={3}>
        <KpiCard label="Depreciación anual" value={k.depreciacionAnual != null ? fmt(k.depreciacionAnual) : '—'} sub={analisis.depreciacion_tipo ?? 'lineal'} tooltip="Pérdida contable anual estimada del valor del activo según su vida útil. No es una salida de caja." />
        <KpiCard label="Valor residual del activo" value={k.valorResidual != null ? fmt(k.valorResidual) : '—'} sub={k.residualPct != null ? `${k.residualPct}% del coste` : undefined} tooltip="Valor estimado del activo al final de su vida útil." />
        <KpiCard label="Vida útil" value={k.vidaUtilUsada != null ? `${k.vidaUtilUsada} años` : '—'} />
      </KpiGrid>
      <SectionHint>La depreciación se muestra como referencia contable. Sin fiscalidad activada, no afecta al VAN ni a la TIR.</SectionHint>
    </Collapsible>
  );
}
...
function SupuestosBlock({ k }: { k: KpisCapexInterno }) {
  return (
    <Collapsible title="Supuestos" subtitle="Tasa, vida útil y residual usados">
      <KpiGrid cols={3}>
        <KpiCard label="Tasa de descuento" value={k.tasaUsada != null ? fmtPct(k.tasaUsada) : '—'} sub="Para descontar la caja" />
        <KpiCard label="Vida útil" value={k.vidaUtilUsada != null ? `${k.vidaUtilUsada} años` : '—'} sub="Horizonte del análisis" />
        <KpiCard label="Valor residual %" value={k.residualPct != null ? `${k.residualPct}%` : '—'} sub={k.residualPorDefecto ? '0% por defecto (conservador)' : 'Del coste CAPEX'} />
      </KpiGrid>
    </Collapsible>
  );
}
...
export function CapexView({ analisis, proyecto }: Props) {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="space-y-2">
        ...
        <KpiGrid cols={4}>
          ...
          <KpiCard
            ...
            sub={k.tasaUsada != null ? `Tasa ${fmtPct(k.tasaUsada)}${k.vidaUtilUsada ? ` · ${k.vidaUtilUsada} años` : ''}` : undefined}
          ...
          <KpiCard
            ...
            sub={k.residualPorDefecto ? '0% (supuesto conservador)' : k.residualPct != null ? `${k.residualPct}% del coste` : undefined}
      ...
      {!tieneInversion && (
        <SectionHint>Falta la inversión CAPEX: el payback, el VAN y la TIR no son definitivos. Añádela en "Editar parámetros" → "Inversión".</SectionHint>
      )}
      {tieneInversion && !tieneAhorro && (
        <SectionHint>Falta el beneficio/ahorro anual: el payback, el VAN y la TIR no son definitivos. Añádelo en "Editar parámetros" → "Beneficio / ahorro".</SectionHint>
      )}
      {tieneInversion && tieneAhorro && sinVida && (
        <SectionHint>Falta la vida útil: sin horizonte no hay VAN ni TIR definitivos. Añádela en "Editar parámetros" → "Vida útil".</SectionHint>
      )}
      {k.cajaNetaNegativa && (
        <SectionHint>La caja neta anual es ≤ 0: el CAPEX de mantenimiento iguala o supera el ahorro. El payback no es definitivo y conviene revisar la operación.</SectionHint>
      )}
      {k.tirNoConverge && !k.cajaNetaNegativa && (
        <SectionHint>Los flujos no recuperan la inversión: la TIR no es calculable. Revisa el ahorro, la vida útil y el valor residual.</SectionHint>
      )}
