...
function DesgloseBlock({ analisis, k }: { analisis: AnalisisFinanciero; k: KpisCompraVenta }) {
  ...
  return (
    <Collapsible
      ...
      subtitle="De la compra al beneficio neto, paso a paso"
    >
      <div className="space-y-0.5">
        ...
        {k.contingencia != null && k.contingencia > 0 && (
          <DesgloseRow label="Contingencia de obra" value={fmt(k.contingencia)} isSub negativo />
        )}
        {iva > 0 && <DesgloseRow label="IVA no recuperable" value={fmt(iva)} isSub negativo />}
        {costeFin > 0 && (
          <DesgloseRow label="Coste financiero de la deuda" value={fmt(costeFin)} isSub negativo />
        )}
        ...
        {gastosVta > 0 && (
          <DesgloseRow label="Gastos de venta" value={fmt(gastosVta)} isSub negativo />
        )}
        {k.comisionAgente != null && k.comisionAgente > 0 && (
          <DesgloseRow label="Comisión de agente" value={fmt(k.comisionAgente)} isSub negativo />
        )}
        <DesgloseRow label="Beneficio antes de impuestos" value={safe(k.margenBruto, fmt)} bold />
        {k.impuestoSociedades != null && (
          <DesgloseRow
            label="Impuesto de sociedades estimado"
            value={fmt(k.impuestoSociedades)}
            isSub
            negativo
          />
        )}
      ...
      <SectionHint>
        Para el margen se suman todos los costes. Para TIR/VAN se respeta el momento de cada flujo:
        los gastos de venta, la comisión y los impuestos se pagan a la salida, no en el año 0.
      </SectionHint>
    </Collapsible>
  );
}
...
function FinanciacionBlock({ analisis, k }: { analisis: AnalisisFinanciero; k: KpisCompraVenta }) {
  ...
  return (
    <Collapsible
      title="Financiación"
      subtitle="Préstamo promotor"
      badge={
        k.ltvDeudaBancaria != null ? (
          <span className="text-xs text-zinc-400">LTV {fmtPct(k.ltvDeudaBancaria)}</span>
        ) : undefined
      }
    >
      <KpiGrid cols={4}>
        <KpiCard
          label="Deuda bancaria / promotora"
          value={fmt(deuda)}
          sub="Se devuelve a la venta"
          tooltip="Importe del préstamo promotor. Se devuelve íntegro con el precio de venta (una sola vez)."
        />
        {k.equity != null && (
          <KpiCard
            label="Capital propio aportado"
            value={fmt(k.equity)}
            sub="Costes iniciales − deuda"
            tooltip="Capital propio realmente desembolsado durante la operación."
          />
        )}
        {k.ltvDeudaBancaria != null && (
          <KpiCard
            label="Endeudamiento bancario sobre coste"
            value={fmtPct(k.ltvDeudaBancaria)}
            sub="Deuda promotora / coste total"
            semaforo={semaforoLTV(k.ltvDeudaBancaria)}
            tooltip="Deuda bancaria dividida entre el coste total del proyecto."
          />
        )}
        {analisis.coste_financiero_deuda != null && analisis.coste_financiero_deuda > 0 && (
          <KpiCard
            label="Coste financiero de la deuda"
            value={fmt(analisis.coste_financiero_deuda)}
            sub="Intereses del préstamo"
            tooltip="Intereses totales del préstamo promotor durante la operación."
          />
        )}
      </KpiGrid>
    </Collapsible>
...
function SensibilidadVentaBlock({ analisis, proyecto }: Props) {
  ...
  const variaciones = [-0.1, -0.05, 0, 0.05];
  const rows = variaciones.map((d) => {
    const k = calcKpisCV(
      { ...analisis, precio_venta_previsto: venta * (1 + d) },
      proyecto.fecha_inicio,
      proyecto.fecha_prevista_salida,
      analisis.superficie_arrendable_m2 ?? null,
    );
    return { d, beneficio: k.beneficioNeto, margen: k.margenPct, van: k.van, tir: k.tirEquity };
  });
  return (
    <Collapsible
      title="Sensibilidad a precio de venta"
      subtitle="Qué pasa si el precio de venta varía"
    >
          ...
          <thead>
            <tr className="border-b border-white/[0.05]">
              {['Precio venta', 'Beneficio neto', 'Margen', 'VAN', 'TIR'].map((h) => (
                <th
                  key={h}
                  className="text-2xs whitespace-nowrap px-3 py-2 text-left font-semibold uppercase tracking-widest text-zinc-600"
                >
                  {h}
                </th>
              ))}
          ...
          <tbody>
            {rows.map((r) => {
              ...
              return (
                <tr
                  key={r.d}
                  className={`border-b border-white/[0.03] last:border-0 ${r.d === 0 ? 'bg-white/[0.02]' : ''}`}
                >
                  ...
                  <td
                    className={`px-3 py-2 tabular-nums ${malo ? 'text-rose-400' : 'text-zinc-300'}`}
                  >
                    {r.beneficio != null ? fmt(r.beneficio) : '—'}
                  </td>
                  <td className="px-3 py-2 tabular-nums text-zinc-300">
                    {r.margen != null ? fmtPct(r.margen) : '—'}
                  </td>
                  <td
                    className={`px-3 py-2 tabular-nums ${r.van != null && r.van < 0 ? 'text-rose-400' : 'text-zinc-300'}`}
                  >
                    {r.van != null ? fmt(r.van) : '—'}
                  </td>
                  <td className="px-3 py-2 tabular-nums text-zinc-300">
                    {r.tir != null ? fmtPct(r.tir) : '—'}
                  </td>
                </tr>
              );
            })}
...
function CosteM2Block({ analisis, k }: { analisis: AnalisisFinanciero; k: KpisCompraVenta }) {
  ...
  return (
    <Collapsible title="Coste / m²" subtitle={`${m2.toLocaleString('es-ES')} m²`}>
      <KpiGrid cols={3}>
        <KpiCard
          label="Coste por metro cuadrado"
          value={fmt(k.costeM2)}
          sub="Coste total / superficie"
          tooltip="Coste total del proyecto dividido entre la superficie. Sirve para comparar promociones similares."
        />
        {ventaM2 != null && (
          <KpiCard
            label="Precio de venta por m²"
            value={fmt(ventaM2)}
            sub="Precio de venta / superficie"
          />
        )}
      </KpiGrid>
    </Collapsible>
...
export function CompraVentaView({ analisis, proyecto }: Props) {
  ...
  const kPes = calcKpisCV(
    { ...analisis, precio_venta_previsto: venta * 0.9 },
    proyecto.fecha_inicio,
    proyecto.fecha_prevista_salida,
    analisis.superficie_arrendable_m2 ?? null,
  );
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="space-y-2">
        ...
        <KpiGrid cols={4}>
          ...
          <KpiCard
            ...
            sub={
              k.inversionTotal != null && tieneVenta
                ? `Coste total: ${fmt(k.inversionTotal)}`
      ...
      {k.equityInsuficiente && (
        <SectionHint>
          Capital aportado insuficiente o negativo (la deuda iguala o supera los costes iniciales):
          revisar estructura de financiación. La TIR sobre capital propio no es fiable en este caso.
        </SectionHint>
      )}
      {!tieneVenta && (
        <SectionHint>
          Falta el precio de venta previsto: el beneficio, la TIR y el VAN no son definitivos.
          Complétalo en "Editar parámetros" → bloque "Venta".
        </SectionHint>
      )}
      {tieneVenta && !tienePlazo && (
        <SectionHint>
          Falta el plazo de la operación: la TIR y el VAN no son definitivos. Añade fechas o el
          plazo total en "Editar parámetros" → "Horizonte y salida".
        </SectionHint>
      )}
      {k.usaFallbackObra && tienePlazo && (
        <SectionHint>
          TIR/VAN calculadas usando el plazo de obra como estimación. Para mayor precisión,
          introduce el plazo total de la operación.
        </SectionHint>
      )}
