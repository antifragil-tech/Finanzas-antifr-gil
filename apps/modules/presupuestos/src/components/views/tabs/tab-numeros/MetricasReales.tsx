...
import {
  getMetricasProyectoResumen,
  getFlujosProyectoConsolidados,
} from '../../../../lib/metricasProyectoApi';
...
function FlujosChart({ flujos }: { flujos: FlujoProyectoConsolidado[] }) {
  ...
  const data = sorted.reduce<ChartRow[]>((acc, f) => {
    ...
    acc.push({
      ...
      importe: Math.round(f.importe),
      acumulado: Math.round(prev + f.importe),
    });
    return acc;
  }, []);
  ...
  return (
    <div className="rounded-2xl border border-white/[0.06] bg-zinc-900/30 p-5">
      <p className="text-2xs mb-4 font-semibold uppercase tracking-widest text-zinc-500">
        Flujos y acumulado
      </p>
    ...
    <div className="overflow-hidden rounded-2xl border border-white/[0.06] bg-zinc-900/30">
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="border-b border-white/[0.05]">
              {['Fecha', 'Concepto', 'Importe', 'Fuente', 'Estado', 'Tipo', 'Sociedad'].map((h) => (
                <th
                  key={h}
                  className="text-2xs whitespace-nowrap px-4 py-2.5 text-left font-semibold uppercase tracking-widest text-zinc-600"
                >
                  {h}
                </th>
              ))}
          ...
          <tbody>
            {visible.map((f, i) => (
              ...
              >
                <td className="whitespace-nowrap px-4 py-2.5 text-zinc-400">{fmtDate(f.fecha)}</td>
                <td className="max-w-[180px] truncate px-4 py-2.5 text-zinc-300">
                  {f.concepto ?? '—'}
                </td>
                <td
                  className={`whitespace-nowrap px-4 py-2.5 font-medium tabular-nums ${f.importe < 0 ? 'text-rose-400' : 'text-emerald-400'}`}
                >
                ...
                <td className="px-4 py-2.5">
                  <span
                    className={`text-2xs rounded-full border px-2 py-0.5 font-medium ${FUENTE_COLOR[f.fuente] ?? 'border-white/10 bg-zinc-800 text-zinc-400'}`}
                  >
                    {FUENTE_LABEL[f.fuente] ?? f.fuente}
                  </span>
                </td>
                <td className="px-4 py-2.5 capitalize text-zinc-500">{f.estado ?? '—'}</td>
                <td className="px-4 py-2.5 text-zinc-500">
                  {f.es_real ? (
                    <span className="text-emerald-500">Real</span>
                  ) : (
                    <span className="text-amber-500">Previsto</span>
                  )}
                </td>
      ...
      {flujos.length > 8 && (
        <button
          ...
          className="flex w-full items-center justify-center gap-1.5 border-t border-white/[0.05] py-2.5 text-xs text-zinc-500 transition-colors hover:text-zinc-300"
        >
          {expanded ? (
            <>
            ...
            <>
              <ChevronDown size={13} /> Ver todos ({flujos.length})
            </>
          )}
        </button>
      )}
...
export function MetricasReales({ proyectoId }: { proyectoId: string }) {
  ...
  useEffect(() => {
    setLoading(true);
    setError('');
    Promise.all([getMetricasProyectoResumen(proyectoId), getFlujosProyectoConsolidados(proyectoId)])
      .then(([m, f]) => {
        setMetricas(m);
        setFlujos(f);
      })
      ...
      {noHayMetricas && noHayFlujos ? (
        <div className="flex flex-col items-center justify-center rounded-2xl border border-white/[0.06] bg-zinc-900/20 py-10">
          <p className="text-sm text-zinc-500">
            No hay flujos reales vinculados a este proyecto todavía.
          </p>
          <p className="mt-1 text-xs text-zinc-700">
            Vincula facturas, pagos de presupuesto o añade flujos manuales para ver métricas.
          </p>
        </div>
      ) : (
        <>
            ...
            <>
              <KpiGrid cols={4}>
                ...
                <KpiCard
                  ...
                  value={
                    metricas.ingreso_real_acumulado ? fmt(metricas.ingreso_real_acumulado) : '—'
                  }
                ...
                <KpiCard
                  ...
                  semaforo={
                    metricas.saldo_neto_real != null
                      ? metricas.saldo_neto_real >= 0
                        ? 'verde'
                        : 'rojo'
                      : 'neutro'
                  }
              ...
              {metricas.desviacion_presupuesto_importe != null ? (
                <div className="space-y-3 rounded-xl border border-white/[0.06] bg-zinc-900/40 p-4">
                  <p className="text-2xs font-semibold uppercase tracking-widest text-zinc-500">
                    Desviación presupuesto vs real
                  </p>
                  <KpiGrid cols={3}>
                    <KpiCard
                      ...
                      value={
                        metricas.presupuesto_total_aprobado
                          ? fmt(metricas.presupuesto_total_aprobado)
                          : '—'
                      }
                      sub="Total aprobado"
                    ...
                    <KpiCard
                      label="Gasto real acumulado"
                      value={
                        metricas.gasto_real_acumulado ? fmt(metricas.gasto_real_acumulado) : '—'
                      }
                      sub="Flujos reales negativos"
                      tooltip="Total de pagos y salidas de dinero reales vinculadas a este proyecto."
                    />
                    <KpiCard
                      ...
                      sub={
                        metricas.desviacion_presupuesto_pct != null
                          ? `${(metricas.desviacion_presupuesto_pct * 100).toFixed(1)}% sobre presupuesto`
                          : undefined
                      }
                      semaforo={
                        metricas.desviacion_presupuesto_pct == null
                          ? 'neutro'
                          : metricas.desviacion_presupuesto_pct > 0.15
                            ? 'rojo'
                            : metricas.desviacion_presupuesto_pct > 0.05
                              ? 'ambar'
                              : 'verde'
                      }
                ...
                <div className="flex items-center gap-2 rounded-xl border border-white/[0.05] bg-zinc-900/30 px-4 py-3 text-xs text-zinc-600">
                  <span className="text-zinc-700">—</span>
                  Este proyecto todavía no tiene presupuesto maestro. Marca uno para activar la
                  desviación presupuesto vs real.
                </div>
              )}
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  {metricas.inversion_total_proyectada && metricas.gasto_real_acumulado ? (
                    <KpiCard
                      ...
                      value={fmtPct(
                        metricas.gasto_real_acumulado / metricas.inversion_total_proyectada,
                      )}
                      sub="Gasto real / inversión proyectada"
                      semaforo={
                        metricas.gasto_real_acumulado / metricas.inversion_total_proyectada > 1.15
                          ? 'rojo'
                          : metricas.gasto_real_acumulado / metricas.inversion_total_proyectada >
                              1.05
                            ? 'ambar'
              ...
              {(metricas.facturas_pendientes_importe || metricas.facturas_pagadas_importe) && (
                <div className="grid grid-cols-2 gap-3">
                  {metricas.facturas_pendientes_importe != null &&
                    metricas.facturas_pendientes_importe > 0 && (
                      <KpiCard
                  ...
                  {metricas.facturas_pagadas_importe != null &&
                    metricas.facturas_pagadas_importe > 0 && (
                      <KpiCard
