...
function CuadroBlock({ analisis, k }: { analisis: AnalisisFinanciero; k: KpisPrestamo }) {
    ...
    return (
      <Collapsible title="Cuadro de amortización" subtitle="Calendario de cobros">
        <p className="text-xs text-zinc-600">
          Falta el tipo de interés o el plazo para generar el cuadro de amortización.
        </p>
      </Collapsible>
    );
  }
  return (
    <Collapsible
      ...
      subtitle={`${SISTEMA_LABEL[k.sistema] ?? k.sistema} · ${k.plazoMeses ?? '—'} meses`}
    >
      <KpiGrid cols={4}>
        <KpiCard
          label="Capital prestado"
          value={analisis.nominal_prestamo != null ? fmt(analisis.nominal_prestamo) : '—'}
          sub="Desembolso en el año 0"
        />
        <KpiCard
          label="Cuota estimada"
          value={k.cuotaMensual != null ? fmt(k.cuotaMensual) : '—'}
          sub={`${SISTEMA_LABEL[k.sistema] ?? k.sistema} · mensual`}
          tooltip="Pago periódico estimado según el sistema de amortización."
        />
        <KpiCard
          label="Ingreso financiero esperado"
          value={k.interesTotales != null ? fmt(k.interesTotales) : '—'}
          sub="Suma de intereses cobrados"
          tooltip="Intereses totales que se cobran durante la vida del préstamo."
        />
        <KpiCard
          label="Cobro anual"
          value={k.cobroAnual != null ? fmt(k.cobroAnual) : '—'}
          sub="Cuota × 12"
        />
      </KpiGrid>
      <SectionHint>
        El VAN base descuenta este calendario de cobros a la tasa exigida. La garantía no se suma al
        VAN: solo mide cobertura.
      </SectionHint>
    </Collapsible>
  );
}
    ...
    <Collapsible
      ...
      badge={
        k.coberturaGarantia != null ? (
          <span className="text-xs text-zinc-400">Cobertura {fmtX(k.coberturaGarantia)}</span>
        ) : undefined
      }
    >
      {k.coberturaGarantia != null ? (
        <KpiGrid cols={3}>
          <KpiCard
            ...
            value={analisis.valor_garantia != null ? fmt(analisis.valor_garantia) : '—'}
            sub="Valor del colateral"
          />
          <KpiCard
            label="Capital prestado"
            value={analisis.nominal_prestamo != null ? fmt(analisis.nominal_prestamo) : '—'}
          ...
          <KpiCard
            ...
            value={fmtX(k.coberturaGarantia)}
            sub="Valor garantía / capital"
            semaforo={semaforoCobertura(k.coberturaGarantia)}
            tooltip="Valor de la garantía dividido entre el capital prestado. ≥1,30 holgada · 1-1,30 razonable · <1 insuficiente. No se suma al VAN: solo mide cobertura."
          />
        </KpiGrid>
      ) : (
        <p className="text-xs text-zinc-600">
      ...
      <SectionHint>
        La garantía no se trata como cobro automático ni se asume su ejecución al 100%: solo indica
        cobertura frente al capital.
      </SectionHint>
    </Collapsible>
  );
}
...
function RiesgoBlock({ analisis, k }: { analisis: AnalisisFinanciero; k: KpisPrestamo }) {
  return (
    <Collapsible title="Riesgo e impago" subtitle="Pérdida esperada y VAN ajustado">
      {k.perdidaIncompleta && (
        <SectionHint>
          Riesgo incompleto: falta la probabilidad de impago (PD) o la LGD. La pérdida esperada y el
          VAN ajustado no son definitivos.
        </SectionHint>
      )}
      <KpiGrid cols={4}>
        <KpiCard
          ...
          value={
            analisis.probabilidad_impago_pct != null ? `${analisis.probabilidad_impago_pct}%` : '—'
          }
          sub="PD"
          semaforo={
            analisis.probabilidad_impago_pct != null
              ? semaforoProbImpago(analisis.probabilidad_impago_pct / 100)
              : 'neutro'
          }
        ...
        <KpiCard
          label="LGD"
          value={analisis.lgd_pct != null ? `${analisis.lgd_pct}%` : '—'}
          sub="Pérdida dado el impago"
          tooltip="Porcentaje del capital que se pierde si se produce el impago (Loss Given Default)."
        />
        <KpiCard
          label="Pérdida esperada"
          value={k.perdidaEsperada != null ? fmt(k.perdidaEsperada) : '—'}
          sub="Capital × PD × LGD"
          tooltip="Pérdida estadística esperada por riesgo de impago."
        />
        <KpiCard
          label="VAN ajustado por riesgo"
          value={k.vanAjustado != null ? fmt(k.vanAjustado) : '—'}
          sub="VAN base − VP(pérdida esperada)"
          semaforo={k.vanAjustado != null ? (k.vanAjustado > 0 ? 'verde' : 'rojo') : 'neutro'}
          tooltip="VAN base menos el valor presente de la pérdida esperada. No mezcla rentabilidad contractual y riesgo en una sola cifra: se muestran ambas lecturas."
        />
        {analisis.rating_interno && (
          <KpiCard label="Rating interno" value={analisis.rating_interno} semaforo="neutro" />
        )}
      </KpiGrid>
    </Collapsible>
  );
    ...
    <Collapsible title="Supuestos" subtitle="Tasa, plazo y sistema usados">
      <KpiGrid cols={3}>
        <KpiCard
          ...
          sub="Para descontar los cobros"
        />
        <KpiCard
          label="Plazo"
          value={k.plazoMeses != null ? `${k.plazoMeses} meses` : '—'}
          sub={k.plazoMeses != null ? `${(k.plazoMeses / 12).toFixed(1)} años` : undefined}
        />
...
export function PrestamoView({ analisis, proyecto }: Props) {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="space-y-2">
        ...
        <KpiGrid cols={4}>
          ...
          <KpiCard
            ...
            sub={
              k.tasaDescuentoUsada != null ? `Base · tasa ${fmtPct(k.tasaDescuentoUsada)}` : 'Base'
            }
          ...
          <KpiCard
            ...
            value={
              analisis.probabilidad_impago_pct != null
                ? `${analisis.probabilidad_impago_pct}%`
                : '—'
            }
      ...
      {!tieneCapital && (
        <SectionHint>
          Falta el capital prestado: la cuota, el VAN y la TIR no son definitivos. Complétalo en
          "Editar parámetros" → "Capital".
        </SectionHint>
      )}
      {tieneCapital && sinTipo && (
        <SectionHint>
          Falta el tipo de interés: los intereses y la TIR no son fiables. Añádelo en "Editar
          parámetros" → "Tipo y plazo".
        </SectionHint>
      )}
      {tieneCapital && !sinTipo && sinPlazo && (
        <SectionHint>
          Falta el plazo: sin cuadro de amortización no hay VAN ni TIR definitivos. Añádelo en
          "Editar parámetros" → "Tipo y plazo".
        </SectionHint>
      )}
      {k.tirNoConverge && (
        <SectionHint>
          Los cobros no recuperan el capital prestado: la TIR no es calculable. Revisa tipo, plazo y
