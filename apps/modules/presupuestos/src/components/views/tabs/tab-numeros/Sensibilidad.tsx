...
function buildSensRow(
  ...
  const deltaRel = relPct != null ? `${relPct >= 0 ? '+' : ''}${relPct.toFixed(0)}%` : '—';
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-white/[0.06] bg-zinc-900/30">
      <div className="border-b border-white/[0.05] px-5 py-3">
        <p className="text-2xs font-semibold uppercase tracking-widest text-zinc-500">{title}</p>
        <p className="text-2xs mt-0.5 text-zinc-700">
          Simula el impacto de cambios adversos en las variables clave. La columna "Diferencia"
          muestra el cambio absoluto; "Impacto" muestra la variación relativa respecto al escenario
          base.
        </p>
      </div>
        ...
        <table className="w-full min-w-[640px] text-xs">
          <thead>
            <tr className="border-b border-white/[0.05]">
              {(
                ...
                <th
                  key={h}
                  className="text-2xs whitespace-nowrap px-4 py-2 text-left font-semibold uppercase tracking-widest text-zinc-600"
                >
                  {h}
                </th>
              ))}
          ...
          <tbody>
            {rows.map((r, i) => (
              ...
              >
                <td className="whitespace-nowrap px-4 py-2.5 text-zinc-400">{r.escenario}</td>
                ...
                <td
                  className={`whitespace-nowrap px-4 py-2.5 font-medium tabular-nums ${r.esNegativo ? 'text-rose-400' : 'text-emerald-400'}`}
                >
                  {r.deltaAbs}
                </td>
                <td
...
function calcRentaRows(a: AnalisisFinanciero, fi: string | null, fs: string | null): SensRow[] {
  ...
  const s1 = calcKpisRentaExtended(
    { ...a, renta_mensual_bruta: pct(a.renta_mensual_bruta, -0.1) },
    fi,
    fs,
  );
  const s2 = calcKpisRentaExtended(
    {
      ...
      ibi_anual: pct(a.ibi_anual, 0.1),
      seguro_anual: pct(a.seguro_anual, 0.1),
    },
    fi,
    fs,
  );
...
function calcCVRows(a: AnalisisFinanciero, fi: string | null, fs: string | null): SensRow[] {
  ...
  const s1 = calcKpisCV(
    { ...a, precio_venta_previsto: pct(a.precio_venta_previsto, -0.05) },
    fi,
    fs,
    sup,
  );
  const s2 = calcKpisCV(
    { ...a, precio_venta_previsto: pct(a.precio_venta_previsto, -0.1) },
    fi,
    fs,
    sup,
  );
  const s3 = calcKpisCV({ ...a, presupuesto_obra: pct(a.presupuesto_obra, 0.1) }, fi, fs, sup);
...
function calcExplotacionRows(a: AnalisisFinanciero): SensRow[] {
  ...
  const s1 = calcKpisExplotacion({
    ...a,
    ingresos_anuales_previstos: pct(a.ingresos_anuales_previstos, -0.1),
  });
  const s2 = calcKpisExplotacion({ ...a, costes_fijos_anuales: pct(a.costes_fijos_anuales, 0.1) });
...
function calcPrestamoRows(a: AnalisisFinanciero): SensRow[] {
  ...
  const s1 = calcKpisPrestamo({
    ...a,
    probabilidad_impago_pct:
      a.probabilidad_impago_pct != null ? a.probabilidad_impago_pct * 2 : null,
  });
  const s2 = calcKpisPrestamo({
    ...a,
    lgd_pct: a.lgd_pct != null ? Math.min(100, a.lgd_pct + 10) : null,
  });
  return [
    buildSensRow(
      'Probabilidad impago ×2',
      'Pérdida esperada',
      b.perdidaEsperada,
      s1.perdidaEsperada,
      'eur',
...
function calcCapexRows(a: AnalisisFinanciero): SensRow[] {
  ...
  const s1 = calcKpisCapexInterno({
    ...a,
    beneficio_esperado_anual: pct(a.beneficio_esperado_anual, -0.2),
  });
  const s2 = calcKpisCapexInterno({ ...a, precio_adquisicion: pct(a.precio_adquisicion, 0.1) });
  const s3 = calcKpisCapexInterno({
    ...a,
    vida_util_activo_anios:
      a.vida_util_activo_anios != null ? Math.max(1, a.vida_util_activo_anios - 1) : null,
  });
  return [
    buildSensRow(
      'Beneficio −20%',
      'Recuperación del CAPEX',
      b.paybackCapex,
      s1.paybackCapex,
      'ano',
      true,
...
function calcAlternativoRows(
  a: AnalisisFinanciero,
  fi: string | null,
  fs: string | null,
): SensRow[] {
  const b = calcKpisAlternativo(a, fi, fs);
  const s1 = calcKpisAlternativo(
    { ...a, retorno_previsto_total: pct(a.retorno_previsto_total, -0.2) },
    fi,
    fs,
  );
  return [buildSensRow('Retorno −20%', 'Múltiplo sobre capital (MOIC)', b.moic, s1.moic, 'mult')];
}
...
export function Sensibilidad({
  ...
  return <SensTable title="Análisis de sensibilidad — ¿Qué pasa si...?" rows={rows} />;
}
