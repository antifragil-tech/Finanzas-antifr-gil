...
import type { AnalisisFinanciero, KpisAlternativo, Semaforo } from '../../../../lib/analisisFinanciero';
import { calcKpisAlternativo, semaforoTir } from '../../../../lib/analisisFinanciero';
import { evaluarCalidadAlternativo, evaluarVeredictoAlternativo } from '../../../../lib/alternativoInsights';
  ...
  return (
    <Collapsible title="Detalle de la operación" subtitle="Descripción y retorno esperado">
      {analisis.descripcion_operacion && (
        <p className="text-sm text-zinc-400 bg-zinc-900/40 border border-white/[0.06] rounded-xl px-4 py-3 mb-3">{analisis.descripcion_operacion}</p>
      )}
      <KpiGrid cols={3}>
        <KpiCard label="Capital requerido" value={k.capitalRequerido != null ? fmt(k.capitalRequerido) : '—'} sub="Capital invertido (año 0)" />
        <KpiCard label="Retorno esperado total" value={k.retornoTotal != null ? fmt(k.retornoTotal) : '—'} sub="Total a cobrar a la salida (capital + plusvalía)" tooltip="Importe TOTAL esperado a cobrar a la salida, incluyendo la recuperación del capital más la plusvalía. No es solo el beneficio." />
        <KpiCard label="Beneficio esperado" value={k.beneficioEsperado != null ? fmt(k.beneficioEsperado) : '—'} sub="Retorno total − capital" tooltip="Ganancia neta esperada: retorno total menos el capital invertido." />
      </KpiGrid>
      <SectionHint>El "retorno esperado total" es lo que se cobra a la salida (recuperación de capital + plusvalía), no solo el beneficio. El beneficio = retorno − capital.</SectionHint>
    </Collapsible>
  );
}
...
function RiesgoLiquidezBlock({ analisis }: { analisis: AnalisisFinanciero }) {
  ...
  return (
    <Collapsible title="Riesgo y liquidez" subtitle="Perfil cualitativo de la operación">
      <KpiGrid cols={2}>
        <KpiCard label="Nivel de riesgo" value={riesgo ? RIESGO_LABEL[riesgo] ?? riesgo : 'No informado'} semaforo={semaforoRiesgo(riesgo)} tooltip="Bajo: operación defensiva · Medio: riesgo normal · Alto: incertidumbre elevada (ejecución, contraparte, mercado o salida)." />
        <KpiCard label="Nivel de liquidez" value={liquidez ? LIQUIDEZ_LABEL[liquidez] ?? liquidez : 'No informado'} semaforo={semaforoLiquidez(liquidez)} tooltip="Alta: salida sencilla o instrumento líquido · Media: salida posible pero no inmediata · Baja: capital bloqueado o salida incierta." />
      </KpiGrid>
      {(!riesgo || !liquidez) && (
        <SectionHint>Riesgo o liquidez sin informar: baja la calidad del dato y el veredicto se vuelve más prudente. Complétalos en "Editar parámetros" → "Riesgo y liquidez".</SectionHint>
      )}
    </Collapsible>
  );
}
...
function ValoracionBlock({ analisis, k }: { analisis: AnalisisFinanciero; k: KpisAlternativo }) {
  ...
  return (
    <Collapsible title="Valoración actual / plusvalía latente" subtitle="Referencia de mercado hoy">
      <KpiGrid cols={3}>
        <KpiCard label="Valoración actual" value={fmt(analisis.valoracion_actual)} sub="Valor de mercado hoy" tooltip="Valoración de mercado actual de la posición. Es una referencia, no necesariamente un cobro realizable de inmediato." />
        <KpiCard label="Capital requerido" value={k.capitalRequerido != null ? fmt(k.capitalRequerido) : '—'} />
        <KpiCard label="Plusvalía latente" value={k.margenLatente != null ? fmt(k.margenLatente) : '—'} sub="Valoración actual − capital" semaforo={k.margenLatente != null ? (k.margenLatente > 0 ? 'verde' : 'rojo') : 'neutro'} tooltip="Diferencia entre la valoración actual y el capital invertido. No se suma al VAN (el VAN usa el retorno esperado a la salida)." />
      </KpiGrid>
      <SectionHint>La plusvalía latente es una referencia de mercado hoy y no se suma al VAN. No se asume que la valoración pueda realizarse de inmediato.</SectionHint>
    </Collapsible>
  );
}
...
function SupuestosBlock({ k }: { k: KpisAlternativo }) {
  return (
    <Collapsible title="Supuestos" subtitle="Tasa y plazo usados">
      <KpiGrid cols={2}>
        <KpiCard label="Tasa de descuento" value={k.tasaUsada != null ? fmtPct(k.tasaUsada) : '—'} sub="Para descontar el retorno" />
        <KpiCard label="Plazo hasta el retorno" value={k.anosUsados != null ? fmtAno(k.anosUsados) : '—'} sub="Entrada → salida" />
      </KpiGrid>
    </Collapsible>
  );
}
...
export function AlternativoView({ analisis, proyecto }: Props) {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="space-y-2">
        ...
        <KpiGrid cols={4}>
          ...
          <KpiCard
            ...
            semaforo={k.beneficioEsperado != null ? (k.beneficioEsperado > 0 ? 'verde' : 'rojo') : 'neutro'}
          ...
          <KpiCard
            ...
            sub="Retorno total / capital"
            semaforo={k.moic != null ? (k.moic >= 2 ? 'verde' : k.moic >= 1.5 ? 'ambar' : 'rojo') : 'neutro'}
          ...
          <KpiCard
            ...
            value={analisis.nivel_riesgo ? (RIESGO_LABEL[analisis.nivel_riesgo] ?? analisis.nivel_riesgo) : 'No informado'}
          ...
          <KpiCard
            ...
            value={analisis.nivel_liquidez ? (LIQUIDEZ_LABEL[analisis.nivel_liquidez] ?? analisis.nivel_liquidez) : 'No informado'}
      ...
      {!tieneCapital && (
        <SectionHint>Falta el capital requerido: el MOIC, el VAN y la TIR no son definitivos. Añádelo en "Editar parámetros" → "Capital".</SectionHint>
      )}
      {tieneCapital && !tieneRetorno && (
        <SectionHint>Falta el retorno esperado total: el MOIC, el VAN y la TIR no son definitivos. Añádelo en "Editar parámetros" → "Retorno esperado".</SectionHint>
      )}
      {tieneCapital && tieneRetorno && !tienePlazo && (
        <SectionHint>Faltan las fechas de inicio y salida del proyecto: la TIR y el VAN no son definitivos.</SectionHint>
      )}
      {sinTasa && tieneCapital && (
        <SectionHint>No se ha indicado tasa de descuento: se usa 8% por defecto. Ajústala en "Editar parámetros" → "Supuestos avanzados".</SectionHint>
      )}
