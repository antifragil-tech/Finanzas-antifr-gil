...
export function evaluarVeredictoCV(
  ...
  const muySensible =
    beneficio != null && benPes != null && beneficio > 0 && benPes < beneficio * 0.4;
  ...
  if (beneficio != null && beneficio <= 0)
    motivosRevisar.push('El beneficio neto es nulo o negativo.');
  if (pesimistaRompe && !sinVenta)
    motivosRevisar.push('En escenario pesimista (venta −10%) la operación entra en pérdidas.');
  ...
  if (tirSuperaTasa && vanPositivo && (muySensible || ltvAlto || plazoLargo)) {
    if (muySensible)
      bullets.push(
        'El resultado depende mucho del precio de venta (cae con fuerza en el pesimista).',
      );
    if (ltvAlto && ltv != null)
      bullets.push(
        `Apalancamiento alto (LTV ${(ltv * 100).toFixed(0)}%): amplifica retorno y riesgo.`,
      );
    if (plazoLargo && anos != null)
      bullets.push(`Plazo largo (${anos.toFixed(1)} años): más exposición a desviaciones.`);
    if (tir != null)
      bullets.push(
    ...
    return {
      tipo: 'Agresivo',
      motivo: 'Buen retorno, pero muy dependiente del precio de venta o del apalancamiento.',
  ...
  if (
    ...
    if (anos != null)
      bullets.push(
        `Plazo ${anos.toFixed(1)} años; el escenario pesimista no destruye la operación.`,
      );
    return {
      tipo: 'Atractivo',
      motivo: 'Rentabilidad por encima del umbral exigido, con margen y beneficio claros.',
      bullets: bullets.slice(0, 5),
    };
  }
  ...
  bullets.push(
    margen != null
      ? `Margen sobre venta moderado (${(margen * 100).toFixed(1)}%).`
      : 'Margen moderado.',
  );
  bullets.push(
    tieneDeuda
  ...
  if (beneficio != null)
    bullets.push(
      beneficio > 0
        ? `Beneficio positivo (${Math.round(beneficio).toLocaleString('es-ES')} €) pero sin gran upside.`
        : 'Beneficio ajustado.',
    );
  return {
    tipo: 'Defensivo',
    motivo: 'Perfil conservador: margen moderado, riesgo y plazo controlados.',
    bullets: bullets.slice(0, 5),
  };
}
...
export function evaluarCalidadCV(a: AnalisisFinanciero): CalidadDato {
  const tieneDeuda = (a.deuda_promotora ?? 0) > 0;
  const tienePlazo =
    !!(a.fecha_inicio_operacion && a.fecha_salida_prevista) ||
  ...
  const defs: DefCampo[] = [
    ...
    },
    {
      ...
      peso: 3,
      critico: true,
      estado: (a) => (tiene(a.presupuesto_obra) ? 'completo' : 'faltante'),
    },
    {
      label: 'Precio de venta previsto',
      peso: 3,
      critico: true,
      estado: (a) => (tiene(a.precio_venta_previsto) ? 'completo' : 'faltante'),
    },
    {
      label: 'Plazo de la operación',
      peso: 2,
      critico: true,
      estado: () => (tienePlazo ? 'completo' : 'faltante'),
    },
    {
    ...
    {
      label: 'Contingencia de obra',
      peso: 1,
      estado: (a) => (a.contingencia_obra_pct != null ? 'completo' : 'estimado'),
    },
    {
      label: 'Gastos de venta',
      peso: 1,
      estado: (a) => (tiene(a.gastos_venta) ? 'completo' : 'faltante'),
    },
    {
      label: 'Comisión de agente',
      peso: 1,
      estado: (a) => (a.comision_agente_pct != null ? 'completo' : 'estimado'),
    },
    {
    ...
    },
    {
      ...
      peso: 1,
      estado: (a) => (a.deuda_promotora != null ? 'completo' : 'estimado'),
    },
    {
      label: 'Superficie (m²)',
      peso: 1,
  ...
  if (tieneDeuda) {
    defs.push({
      ...
      peso: 1,
      estado: (a) => (tiene(a.coste_financiero_deuda) ? 'completo' : 'estimado'),
    });
  }
