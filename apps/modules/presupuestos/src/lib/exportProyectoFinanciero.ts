...
export type CampoCalidadInforme = {
  label: string;
  estado: 'completo' | 'estimado' | 'faltante';
  critico: boolean;
};
...
const SIST_L: Record<string, string> = {
  frances: 'Francés',
  aleman: 'Alemán',
...
function escenarios(
  ...
  p: ProyectoRow,
  filas: { label: string; key: string; render: (n: number) => string }[],
): EscenarioCol[] {
...
function alertasDesde(cal: CalidadDato, extra: string[] = []): string[] {
  const out: string[] = [];
  if (cal.faltantesCriticos.length > 0)
    out.push(`Faltan datos críticos: ${cal.faltantesCriticos.join(', ')}.`);
...
function buildRenta(
  a: AnalisisFinanciero,
  p: ProyectoRow,
  base: Pick<InformeFinanciero, 'nombreProyecto' | 'tipoLabel' | 'sociedad' | 'fechaGeneracion'>,
): InformeFinanciero {
  ...
  return {
    ...base,
    veredictoTipo: ver.tipo,
    veredictoMotivo: ver.motivo,
    veredictoBullets: ver.bullets,
    kpisResumen: [
        ...
        filas: [
          {
            ...
            valor: fmt(a.renta_mensual_bruta, (n) => eur(n) + '/mes'),
          },
          {
            label: 'Ocupación',
            valor: a.tasa_ocupacion_prevista_pct != null ? `${a.tasa_ocupacion_prevista_pct}%` : NI,
          },
      ...
      },
      {
        ...
        filas: [
          { label: 'Deuda hipotecaria', valor: fmt(a.deuda_hipotecaria, eur) },
          {
            ...
            valor: fmt(a.cuota_hipoteca_mensual, (n) => eur(n) + '/mes'),
          },
          {
            label: 'Tipo de interés de deuda',
            valor: a.tipo_interes_deuda_pct != null ? `${a.tipo_interes_deuda_pct}%` : NI,
          },
          {
            label: 'Plazo restante',
            valor: a.plazo_restante_anios != null ? `${a.plazo_restante_anios} años` : NI,
          },
        ],
      },
    ],
      ...
      ? [
          {
            ...
            filas: [
              ...
              {
                label: 'Impuestos',
                valor: f.fiscalidadAplicada ? eur(-f.impuestos) : 'No aplicados',
              },
    ...
    supuestos: [
      { label: 'Tasa de descuento', valor: fmt(k.tasaDescuentoUsada, pct) },
      {
        ...
        valor: k.horizonteUsado != null ? `${k.horizonteUsado} años` : NI,
      },
      { label: 'Método de valor residual', valor: k.metodoResidualLabel ?? NI },
      {
        label: 'Revalorización anual usada',
        valor:
          k.revalorizacionResidualPct != null
            ? `${k.revalorizacionResidualPct}%${k.revalorizacionPorDefecto ? ' (0% por defecto)' : ''}`
            : '—',
      },
      {
        label: 'Fiscalidad',
        valor: k.fiscalidadAplicada ? 'Aplicada' : 'No aplicada (antes de impuestos)',
      },
    ],
    calidadScore: cal.score,
    calidadNivel: cal.nivel,
    calidadCampos: campos(cal),
    alertas: alertasDesde(cal, [
      a.tasa_ocupacion_prevista_pct == null
        ? 'Ocupación no informada: se asume 100% de ocupación.'
        : '',
      a.horizonte_analisis_anios == null ? 'Horizonte no informado: se usan 10 años.' : '',
      k.revalorizacionPorDefecto
        ? 'Revalorización no informada: se usa 0% (supuesto conservador).'
        : '',
    ...
    formulas: [
      {
        nombre: 'NOI',
        formula: 'Renta efectiva menos gastos operativos, IBI, seguro y CAPEX de mantenimiento',
      },
      { nombre: 'Rentabilidad neta sobre coste', formula: 'NOI dividido entre la inversión total' },
      {
        nombre: 'Rentabilidad neta sobre valor actual',
        formula: 'NOI dividido entre el valor actual estimado',
      },
      {
        nombre: 'VAN',
        formula: 'VAN = -inversión inicial + suma de cajas descontadas + valor residual descontado',
      },
      {
        nombre: 'TIR',
        formula: 'Tasa que hace el VAN igual a cero (flujos anuales más valor residual)',
      },
...
function buildCV(
  ...
  const k = calcKpisCV(
    ...
    a.superficie_arrendable_m2 ?? null,
  );
  const venta = a.precio_venta_previsto ?? 0;
  const kPes = calcKpisCV(
    { ...a, precio_venta_previsto: venta * 0.9 },
    p.fecha_inicio,
    p.fecha_prevista_salida,
    a.superficie_arrendable_m2 ?? null,
  );
  ...
  return {
    ...
    datosIntroducidos: [
      {
        ...
        filas: [
          ...
          { label: 'Presupuesto obra / reforma', valor: fmt(a.presupuesto_obra, eur) },
          {
            ...
            valor: a.contingencia_obra_pct != null ? `${a.contingencia_obra_pct}%` : NI,
          },
      ...
      },
      {
        ...
        filas: [
          ...
          { label: 'Gastos de venta', valor: fmt(a.gastos_venta, eur) },
          {
            ...
            valor: a.comision_agente_pct != null ? `${a.comision_agente_pct}%` : NI,
          },
        ],
      },
      {
        titulo: 'Financiación y fiscalidad',
        filas: [
          ...
          { label: 'Coste financiero deuda', valor: fmt(a.coste_financiero_deuda, eur) },
          {
            ...
            valor:
              a.impuesto_sociedades_pct != null
                ? `${a.impuesto_sociedades_pct}%`
                : '26% (por defecto)',
          },
        ],
      },
    ...
    kpisCalculados: [
      ...
      ...(tieneDeuda
        ? [{ label: 'Endeudamiento bancario sobre coste', valor: fmt(k.ltvDeudaBancaria, pct) }]
        : []),
    ...
    alertas: alertasDesde(cal, [
      venta <= 0
        ? 'Falta el precio de venta: el beneficio, la TIR y el VAN no son definitivos.'
        : '',
      k.anosUsados == null
        ? 'Falta el plazo de la operación: la TIR y el VAN no son definitivos.'
        : '',
      k.usaFallbackObra ? 'TIR/VAN calculadas usando el plazo de obra como estimación.' : '',
      k.equityInsuficiente
        ? 'Capital aportado insuficiente o negativo: revisar estructura de financiación.'
        : '',
    ]),
    formulas: [
      {
        nombre: 'Coste total',
        formula:
          'Compra + gastos adq. + obra + contingencia + IVA + financiación + gastos venta + comisión',
      },
      {
        nombre: 'Beneficio neto',
        formula: 'Precio de venta − coste total − impuesto de sociedades',
      },
      {
        nombre: 'Equity (año 0)',
        formula: 'Compra + gastos adq. + obra + contingencia + IVA + financiación − deuda',
      },
      {
        nombre: 'VAN',
        formula:
          'Flujo de salida /(1+tasa)^años − equity, con flujo salida = venta − deuda − gastos venta − comisión − IS',
      },
  ...
  return {
        ...
        filas: [
          {
            ...
            valor: a.costes_variables_pct != null ? `${a.costes_variables_pct}%` : NI,
          },
    ...
    desglose: [
      {
        ...
        filas: [
          ...
          {
            label: 'Impuestos',
            valor: k.fiscalidadAplicada ? fmt(k.impuestosAnual, (n) => eur(-n)) : 'No aplicados',
          },
    ...
    supuestos: [
      ...
      { label: 'Horizonte', valor: k.horizonteUsado != null ? `${k.horizonteUsado} años` : NI },
      {
        ...
        valor: k.crecimientoPct != null ? `${k.crecimientoPct}%` : '—',
      },
      {
        label: 'Valor de continuidad',
        valor: k.valorTerminalAplicado
          ? `${k.metodoTerminalLabel ?? 'Aplicado'} · ${fmt(k.valorTerminalFuturo, eur)}`
          : 'No aplicado (conservador)',
      },
    ...
    alertas: alertasDesde(cal, [
      k.valorTerminalIncompleto
        ? 'El valor de continuidad está activado pero faltan datos: no se incluye en VAN/TIR.'
        : '',
      !k.valorTerminalAplicado
        ? 'Sin valor de continuidad: VAN/TIR conservadores (sin valor residual del negocio).'
        : '',
    ]),
    formulas: [
      ...
      {
        nombre: 'VAN',
        formula: '−inversión + Σ FCF × (1+g)^(t−1)/(1+tasa)^t (+ valor terminal si aplica)',
      },
  ...
  return {
    ...
    kpisResumen: [
      { label: 'Capital prestado', valor: fmt(a.nominal_prestamo, eur) },
      {
        ...
        valor: k.tirNoConverge ? 'No calculable' : fmt(k.tirInstrumento, pct),
      },
    ...
    datosIntroducidos: [
      {
        ...
        filas: [
          ...
          { label: 'Capital prestado', valor: fmt(a.nominal_prestamo, eur) },
          {
            ...
            valor: a.tipo_interes_pct != null ? `${a.tipo_interes_pct}%` : NI,
          },
          { label: 'Plazo', valor: a.plazo_meses != null ? `${a.plazo_meses} meses` : NI },
          {
            label: 'Sistema de amortización',
            valor: SIST_L[a.sistema_amortizacion ?? 'frances'] ?? a.sistema_amortizacion ?? NI,
          },
        ],
      },
      {
        titulo: 'Garantía y riesgo',
        filas: [
          { label: 'Colateral / garantía', valor: a.colateral ?? NI },
          { label: 'Valor de garantía', valor: fmt(a.valor_garantia, eur) },
          {
            label: 'Probabilidad de impago (PD)',
            valor: a.probabilidad_impago_pct != null ? `${a.probabilidad_impago_pct}%` : NI,
          },
    ...
    kpisCalculados: [
      ...
      {
        label: 'Tipo de interés anual',
        valor: a.tipo_interes_pct != null ? `${a.tipo_interes_pct}%` : '—',
      },
      ...
      {
        label: 'Plazo',
        valor: k.plazoMeses != null ? `${(k.plazoMeses / 12).toFixed(1)} años` : '—',
      },
      ...
      { label: 'Pérdida esperada', valor: fmt(k.perdidaEsperada, eur) },
      {
        ...
        valor: a.probabilidad_impago_pct != null ? `${a.probabilidad_impago_pct}%` : '—',
      },
    ],
    escenarios: escenarios(a, p, [
    ...
    alertas: alertasDesde(cal, [
      ...
      k.perdidaIncompleta
        ? 'Falta PD o LGD: la pérdida esperada y el VAN ajustado no son definitivos.'
        : '',
...
function buildCapex(
  ...
  return {
    ...
    datosIntroducidos: [
      {
        titulo: 'Inversión y ahorro',
        filas: [
          { label: 'Inversión CAPEX', valor: fmt(a.precio_adquisicion, eur) },
          {
            label: 'Beneficio / ahorro anual esperado',
            valor: fmt(a.beneficio_esperado_anual, eur),
          },
          { label: 'CAPEX mantenimiento anual', valor: fmt(a.capex_mantenimiento_anual, eur) },
        ],
      },
      {
        titulo: 'Vida útil y residual',
        filas: [
          {
            label: 'Vida útil',
            valor: a.vida_util_activo_anios != null ? `${a.vida_util_activo_anios} años` : NI,
          },
          {
            label: 'Valor residual',
            valor: a.valor_residual_pct != null ? `${a.valor_residual_pct}%` : '0% (por defecto)',
          },
    ...
    supuestos: [
      ...
      { label: 'Vida útil', valor: k.vidaUtilUsada != null ? `${k.vidaUtilUsada} años` : NI },
      {
        ...
        valor:
          k.residualPct != null
            ? `${k.residualPct}%${k.residualPorDefecto ? ' (0% por defecto)' : ''}`
            : '—',
      },
    ],
    calidadScore: cal.score,
    ...
    alertas: alertasDesde(cal, [
      k.cajaNetaNegativa
        ? 'La caja neta anual es ≤ 0: el mantenimiento se come el ahorro; payback no definitivo.'
        : '',
    ...
    formulas: [
      ...
      {
        nombre: 'VAN',
        formula: '−inversión + Σ caja neta /(1+tasa)^t + valor residual /(1+tasa)^vida',
      },
      { nombre: 'TIR', formula: 'Tasa que iguala el VAN a cero' },
    ],
    explotarLiquidar: null,
...
function buildAlternativo(
  ...
  return {
    ...
    datosIntroducidos: [
      ...
      {
        titulo: 'Riesgo y liquidez',
        filas: [
          {
            label: 'Nivel de riesgo',
            valor: a.nivel_riesgo ? (RIESGO_L[a.nivel_riesgo] ?? a.nivel_riesgo) : NI,
          },
          {
            label: 'Nivel de liquidez',
            valor: a.nivel_liquidez ? (LIQ_L[a.nivel_liquidez] ?? a.nivel_liquidez) : NI,
          },
        ],
      },
    ],
    kpisCalculados: [
      ...
      {
        label: 'Nivel de riesgo',
        valor: a.nivel_riesgo ? (RIESGO_L[a.nivel_riesgo] ?? a.nivel_riesgo) : '—',
      },
      {
        label: 'Nivel de liquidez',
        valor: a.nivel_liquidez ? (LIQ_L[a.nivel_liquidez] ?? a.nivel_liquidez) : '—',
      },
    ],
    escenarios: escenarios(a, p, [
    ...
    calidadCampos: campos(cal),
    alertas: alertasDesde(cal, [
      k.beneficioEsperado != null && k.beneficioEsperado <= 0
        ? 'El retorno esperado no supera el capital invertido.'
        : '',
      a.valoracion_actual == null
        ? 'Sin valoración actual: no se calcula la plusvalía latente.'
        : '',
...
export function construirInforme(a: AnalisisFinanciero, proyecto: ProyectoRow): InformeFinanciero {
  ...
  switch (a.tipo_analisis) {
    case 'renta':
      return buildRenta(a, proyecto, portada);
    case 'compra_venta':
      return buildCV(a, proyecto, portada);
    case 'explotacion':
      return buildExplotacion(a, proyecto, portada);
    case 'prestamo':
      return buildPrestamo(a, proyecto, portada);
    case 'capex_interno':
      return buildCapex(a, proyecto, portada);
    case 'alternativo':
      return buildAlternativo(a, proyecto, portada);
    default:
      return buildRenta(a, proyecto, portada);
  }
}
