...
export function calcularPaybackOperativo(inversion: number, fcfAnual: number): number | null {
...
export function calcularCapitalPendiente(cuadro: CuotaAmortizacion[], mesActual: number): number {
...
export function calcularPaybackCapex(inversion: number, beneficioAnual: number): number | null {
...
export function calcularValorResidualCapex(valorActivo: number, valorResidualPct: number): number {
...
export function calcularYieldSobreCoste(noi: number, inversionTotal: number): number | null {
  if (inversionTotal <= 0) return null;
  return noi / inversionTotal;
}

export function calcularYieldSobreValorActual(noi: number, valorActual: number): number | null {
...
export function calcularCosteTotalProyecto(a: {
  ...
  const base =
    (a.precio_adquisicion ?? 0) +
    (a.gastos_adquisicion ?? 0) +
    (a.presupuesto_obra ?? 0) +
    (a.gastos_venta ?? 0) +
...
export function calcularBeneficioEsperado(precioVenta: number, costeTotalProyecto: number): number {
  return precioVenta - costeTotalProyecto;
}

export function calcularMargenSobreVenta(beneficio: number, precioVenta: number): number | null {
  if (precioVenta <= 0) return null;
  return beneficio / precioVenta;
}

export function calcularROI(beneficio: number, inversionTotal: number): number | null {
  if (inversionTotal <= 0) return null;
...
export function calcKpisCV(
  ...
  const NULL_RESULT: KpisCompraVenta = {
    ...
    tirPreTax: null,
    van: null,
    ltvDeudaBancaria: null,
    financiacionExternaTotalPct: null,
    ltv: null,
    costeM2: null,
    anosUsados: null,
    usaFallbackObra: false,
    equity: null,
    equityInsuficiente: false,
    contingencia: null,
    comisionAgente: null,
    costesSalida: null,
    flujoSalidaNeto: null,
  };
  ...
  const comisionAgente =
    venta > 0 && a.comision_agente_pct ? venta * (a.comision_agente_pct / 100) : 0;
  ...
  const beneficioNeto =
    margenBruto != null && impuestoSociedades != null ? margenBruto - impuestoSociedades : null;
  ...
  const flujoSalidaNeto =
    flujoSalidaPreTax != null && impuestoSociedades != null
      ? flujoSalidaPreTax - impuestoSociedades
      : null;
  ...
  const tirPreTax =
    equity > 0 && anosUsados && flujoSalidaPreTax != null && flujoSalidaPreTax > 0
      ? tirDosFlujosAnual(equity, flujoSalidaPreTax, anosUsados)
      : null;
  const tirEquity =
    equity > 0 && anosUsados && flujoSalidaNeto != null && flujoSalidaNeto > 0
      ? tirDosFlujosAnual(equity, flujoSalidaNeto, anosUsados)
      : null;

  const van =
    equity > 0 && anosUsados && flujoSalidaNeto != null
      ? flujoSalidaNeto / (1 + tasa) ** anosUsados - equity
      : null;

  // ── Financiación ────────────────────────────────────────────────────────────
  // LTV sobre coste total con deuda real, no con deuda implícita por % equity
  const anticipoComprador = a.anticipo_comprador ?? 0;
  const ltvDeudaBancaria =
    deudaPromotora > 0 && inversionTotal > 0 ? deudaPromotora / inversionTotal : null;
  const financiacionExternaTotalPct =
    deudaPromotora + anticipoComprador > 0 && inversionTotal > 0
      ? (deudaPromotora + anticipoComprador) / inversionTotal
      : null;
...
export function calcKpisAlternativo(
  a: AnalisisFinanciero,
  fechaInicio: string | null,
  fechaSalida: string | null,
  cobrosRealesAcumulados?: number,
): KpisAlternativo {
  ...
  const tirEquity =
    equity > 0 && anos && retorno > 0 ? tirDosFlujosAnual(equity, retorno, anos) : null;
...
export function calcFinanciacionRenta(
  a: AnalisisFinanciero,
  horizonteAnios: number,
): FinanciacionRenta {
  ...
  if (sistema === 'frances' && interes > 0 && plazoAnios > 0) {
    ...
    return {
      deudaViva: deuda,
      servicioDeudaAnual: cuotaMensual * 12,
      cuotaMensual,
      interesesAnual,
      amortizacionAnual,
      deudaSalida,
  ...
  if (sistema === 'bullet' && interes > 0) {
    ...
    return {
      deudaViva: deuda,
      servicioDeudaAnual: interesesAnual,
      cuotaMensual: interesesAnual / 12,
      interesesAnual,
      amortizacionAnual: 0,
      deudaSalida: deuda,
      ltv,
  ...
  if (cuotaManual > 0) {
    ...
    const amortizacionAnual =
      interesesAnual != null ? Math.max(0, servicio - interesesAnual) : null;
    // Deuda a salida: con amortización conocida se descuenta lineal; si no, conservador = deuda viva.
    const deudaSalida =
      amortizacionAnual != null ? Math.max(0, deuda - amortizacionAnual * horizonteAnios) : deuda;
    return {
      deudaViva: deuda,
      servicioDeudaAnual: servicio,
      cuotaMensual: cuotaManual,
      interesesAnual,
      amortizacionAnual,
      deudaSalida,
      ltv,
      sistema: 'manual',
      estimado: amortizacionAnual == null,
      incompleto: interes <= 0,
    };
  }
  ...
  return {
    deudaViva: deuda,
    servicioDeudaAnual: interesesAnual,
    cuotaMensual: interesesAnual != null ? interesesAnual / 12 : null,
    interesesAnual,
    amortizacionAnual: 0,
    deudaSalida: deuda,
    ltv,
...
export function calcFlujoAnualRenta(
  ...
  const horizonte =
    a.horizonte_analisis_anios ?? (horizonteDeF != null ? Math.round(horizonteDeF) : 10);
  ...
  const baseImponible = fiscalidadAplicada ? noi - (fin.interesesAnual ?? 0) - amortFiscal : null;
  const impuestos =
    fiscalidadAplicada && baseImponible != null ? Math.max(baseImponible, 0) * isPct : 0;
  ...
  return {
    ...
    ibi,
    seguro,
    capexMantenimiento: capexMant,
    noi,
    servicioDeuda,
    intereses: fin.interesesAnual,
    amortizacion: fin.amortizacionAnual,
    cajaAntesImpuestos,
    baseImponible,
    impuestos,
    fiscalidadAplicada,
...
export function calcKpisRentaExtended(
  ...
  const horizonteUsado =
    a.horizonte_analisis_anios ?? (horizonteDeFechas != null ? Math.round(horizonteDeFechas) : 10); // default 10 años
  ...
  const null_result: KpisRentaExtended = {
    inversionTotal: null,
    rentaAnualBruta: null,
    gastosAnualesTotal: null,
    noIAnual: null,
    yieldBruto: null,
    yieldNetoCoste: null,
    yieldNetoValorActual: null,
    cashOnCash: null,
    dscr: null,
    paybackAnos: null,
    margenLatente: null,
    ...
    van: null,
    vanSinResidual: null,
    tasaDescuentoUsada: null,
    diferencialTirTasa: null,
    servicioDeudaAnual: null,
    deudaSalidaEstimada: null,
    financiacionEstimada: false,
    fiscalidadAplicada: false,
    valorResidualUsado: null,
    valorResidualPresente: null,
    revalorizacionResidualPct: null,
    revalorizacionPorDefecto: false,
    horizonteUsado: null,
    metodoResidualLabel,
    tirSinDatos: false,
  };
  ...
  const ltv =
    fin.ltv ?? (deudaHip > 0 && valoracion && valoracion > 0 ? deudaHip / valoracion : null);

  // valorReversion clásico (cap rate): se mantiene para compatibilidad
  const valorReversion =
    noIAnual > 0 && capRata && capRata > 0 ? calcularValorReversion(noIAnual, capRata) : null;
  ...
  const valorResidualPresente =
    valorResidualUsado != null ? valorResidualUsado / Math.pow(1 + tasa, hAnios) : null;
  ...
  const cajaAnualDescuento = flujo
    ? fiscalidadAplicada
      ? flujo.cajaNetaFinal
      : flujo.cajaAntesImpuestos
    : noIAnual - servDeudaAnual;
  // Venta neta al corte: valor residual − deuda a salida − (costes venta + plusvalía si fiscalidad)
  const costesVenta =
    fiscalidadAplicada && valorResidualUsado != null
      ? valorResidualUsado * ((a.costes_venta_pct ?? 0) / 100)
      : 0;
  const baseePlusvalia =
    valorResidualUsado != null ? valorResidualUsado - inversionTotal - costesVenta : 0;
  const impuestoPlusvalia = fiscalidadAplicada
    ? Math.max(baseePlusvalia, 0) * ((a.impuesto_plusvalia_pct ?? 0) / 100)
    : 0;
  const ventaNeta =
    valorResidualUsado != null
      ? valorResidualUsado - (deudaSalida ?? 0) - costesVenta - impuestoPlusvalia
      : null;
  ...
  const tirConResidual =
    tirSinDatos || ventaNeta === null
      ? null
  ...
  const vanSinResidual =
    equity > 0 && horizonteUsado !== null
      ? vanRentaFlujos(equity, cajaAnualDescuento, 0, hAnios, tasa)
      : null;
  ...
  return {
    ...
    diferencialTirTasa,
    servicioDeudaAnual: fin.servicioDeudaAnual,
    deudaSalidaEstimada: deudaSalida,
    financiacionEstimada: fin.estimado,
    fiscalidadAplicada,
    valorResidualUsado,
    valorResidualPresente,
    revalorizacionResidualPct,
    revalorizacionPorDefecto,
    horizonteUsado,
    metodoResidualLabel,
    tirSinDatos,
  };
...
function vanTirExplotacion(
  ...
  horizonte: number,
  tasa: number,
  valorTerminalFuturo: number,
): { van: number | null; tir: number | null } {
  if (inversion <= 0 || horizonte <= 0) return { van: null, tir: null };

  const flujoAno = (t: number) =>
    fcf * Math.pow(1 + g, t - 1) + (t === horizonte ? valorTerminalFuturo : 0);
...
export function calcKpisExplotacion(a: AnalisisFinanciero): KpisExplotacion {
  ...
  const NULO: KpisExplotacion = {
    ...
    rentaMensualMedia: null,
    ebitdaAnual: null,
    margenEbitda: null,
    breakEvenIngresos: null,
    fcfAnual: null,
    paybackOperativo: null,
    ...
    horizonteUsado: null,
    crecimientoPct: null,
    valorTerminalAplicado: false,
    valorTerminalFuturo: null,
    valorTerminalPresente: null,
    metodoTerminalLabel: null,
    valorTerminalIncompleto: false,
  };
  ...
  const paybackOperativo =
    inversion > 0 && fcfAnual > 0 ? calcularPaybackOperativo(inversion, fcfAnual) : null;
  ...
  const valorTerminalPresente = valorTerminalAplicado
    ? valorTerminalFuturo / Math.pow(1 + tasa, horizonte)
    : null;
  ...
  return {
    ...
    crecimientoPct: a.tasa_crecimiento_anual_pct ?? 0,
...
export function calcKpisPrestamo(a: AnalisisFinanciero): KpisPrestamo {
  ...
  const NULO: KpisPrestamo = {
    ...
    interesTotales: null,
    cobroAnual: null,
    perdidaEsperada: null,
    perdidaIncompleta: false,
    tirInstrumento: null,
    tirNoConverge: false,
    van: null,
    vanAjustado: null,
    coberturaGarantia: null,
    garantiaSinValor: false,
    plazoMeses: null,
    tasaDescuentoUsada: null,
    sistema,
    capitalPendienteActual: null,
  };
  if (!nominal) return NULO;

  const cuadro =
    plazoMeses > 0 && tipoInt >= 0
  ...
  const coberturaGarantia =
    valorGar != null && valorGar > 0 && nominal > 0 ? valorGar / nominal : null;
  ...
  const perdidaEsperada =
    tienePD && tieneLGD
      ? calcularPerdidaEsperada(
          nominal,
          (a.probabilidad_impago_pct ?? 0) / 100,
          (a.lgd_pct ?? 0) / 100,
        )
      : null;
  ...
  if (van != null && perdidaEsperada != null) {
    ...
    const vpPerdida =
      plazoAnos > 0 ? perdidaEsperada / Math.pow(1 + tasa, plazoAnos) : perdidaEsperada;
...
function vanTirCapex(
  inversion: number,
  cajaNeta: number,
  vida: number,
  tasa: number,
  valorResidual: number,
): { van: number | null; tir: number | null; noConverge: boolean } {
...
export function calcKpisCapexInterno(a: AnalisisFinanciero): KpisCapexInterno {
  ...
  const NULO: KpisCapexInterno = {
    inversionCapex: null,
    ahorroBruto: null,
    capexMantenimiento: null,
    cajaNetaAnual: null,
    cajaNetaNegativa: false,
    paybackCapex: null,
    vanCapex: null,
    tirCapex: null,
    tirNoConverge: false,
  ...
  const { van, tir, noConverge } =
    inversion > 0 && vidaUtil > 0
  ...
  return {
    ...
    depreciacionAnual:
      inversion > 0 && vidaUtil > 0
        ? calcularDepreciacionAnual(inversion, vidaUtil, vrPct, tipoD)
        : null,
