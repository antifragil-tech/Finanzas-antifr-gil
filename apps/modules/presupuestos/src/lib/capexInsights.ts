...
export function evaluarVeredictoCapex(
  ...
  const vrPresente = k.valorResidual != null && vida != null && tasa != null
    ? k.valorResidual / Math.pow(1 + tasa, vida) : null;
  const dependeResidual = vrPresente != null && van != null && van > 0 && k.inversionCapex != null
  ...
  if (k.cajaNetaNegativa) motivosRevisar.push('La caja neta anual es ≤ 0: el mantenimiento se come el ahorro.');
  ...
  if (k.tirNoConverge) motivosRevisar.push('Los flujos no recuperan la inversión: la TIR no es calculable.');
  ...
  if (tirSuperaTasa && vanPositivo && (dependeResidual || paybackLargo || vidaLarga)) {
    if (dependeResidual) bullets.push('Gran parte del valor descansa en el valor residual del activo.');
    if (paybackLargo && payback != null) bullets.push(`Recuperación lenta (${payback.toFixed(1)} años).`);
    if (vidaLarga && vida != null) bullets.push(`Vida útil larga (${vida} años): el retorno depende de mantener el ahorro mucho tiempo.`);
    if (tir != null) bullets.push(`TIR ${(tir * 100).toFixed(1)}% por encima de la tasa exigida (${(tasa * 100).toFixed(1)}%).`);
    if (cajaOk && cajaNeta != null) bullets.push(`Caja neta anual ${Math.round(cajaNeta).toLocaleString('es-ES')} €.`);
    return { tipo: 'Agresivo', motivo: 'Buen retorno, pero depende del valor residual o de sostener el ahorro mucho tiempo.', bullets: bullets.slice(0, 5) };
  }
  ...
  if (tirSuperaTasa && vanPositivo && cajaOk && paybackBueno) {
    ...
    if (cajaNeta != null) bullets.push(`Caja neta anual positiva (${Math.round(cajaNeta).toLocaleString('es-ES')} €).`);
    bullets.push('Baja dependencia del valor residual.');
    return { tipo: 'Atractivo', motivo: 'Inversión rentable con recuperación rápida y caja neta positiva.', bullets: bullets.slice(0, 5) };
  }

  // Defensivo: ahorro estable, payback moderado, bajo riesgo
  if (cajaNeta != null) bullets.push(`Caja neta anual ${cajaNeta > 0 ? 'positiva' : 'ajustada'} (${Math.round(cajaNeta).toLocaleString('es-ES')} €).`);
  ...
  return { tipo: 'Defensivo', motivo: 'Inversión estable: ahorro sostenido y riesgo operativo bajo.', bullets: bullets.slice(0, 5) };
}
...
export function evaluarCalidadCapex(a: AnalisisFinanciero): CalidadDato {
  const defs: DefCampo[] = [
    { label: 'Inversión CAPEX',          peso: 3, critico: true, estado: a => tiene(a.precio_adquisicion) ? 'completo' : 'faltante' },
    { label: 'Beneficio / ahorro anual', peso: 3, critico: true, estado: a => tiene(a.beneficio_esperado_anual) ? 'completo' : 'faltante' },
    { label: 'Vida útil',                peso: 3, critico: true, estado: a => tiene(a.vida_util_activo_anios) ? 'completo' : 'faltante' },
    { label: 'Tasa de descuento',        peso: 2, critico: true, estado: a => a.tasa_descuento_interna != null || a.tasa_descuento != null ? 'completo' : 'estimado' },
    { label: 'Valor residual %',         peso: 1,                estado: a => a.valor_residual_pct != null ? 'completo' : 'estimado' },
    { label: 'CAPEX de mantenimiento',   peso: 1,                estado: a => a.capex_mantenimiento_anual != null ? 'completo' : 'estimado' },
    { label: 'Método de depreciación',   peso: 1,                estado: a => a.depreciacion_tipo != null ? 'completo' : 'estimado' },
  ];
