...
export function evaluarVeredictoAlternativo(
  ...
  if (sinPlazo)
    motivosRevisar.push('Falta el plazo (fechas de inicio y salida): sin él no hay VAN ni TIR.');
  if (beneficio != null && beneficio <= 0)
    motivosRevisar.push(
      'El retorno esperado no supera el capital invertido: la operación no genera beneficio.',
    );
  ...
  if (riesgoAlto && liquidezBaja && !(tir != null && tir > tasa * 1.5))
    motivosRevisar.push('Riesgo alto y liquidez baja sin un retorno que lo compense con holgura.');
  ...
  if (
    tirSuperaTasa &&
    vanPositivo &&
    (riesgoAlto || liquidezBaja || plazoLargo || riesgoLiquidezSinInformar)
  ) {
    if (riesgoAlto)
      bullets.push('Riesgo alto: la operación depende de ejecución, contraparte o mercado.');
    if (liquidezBaja)
      bullets.push(
        'Liquidez baja: capital bloqueado, salida incierta o dependiente de un evento futuro.',
      );
    if (riesgoLiquidezSinInformar)
      bullets.push(
        'Riesgo o liquidez sin informar: el veredicto es prudente por falta de contexto.',
      );
    if (plazoLargo && anos != null)
      bullets.push(
        `Plazo largo (${anos.toFixed(1)} años) con el retorno concentrado en un único cobro a la salida.`,
      );
    if (tir != null)
      bullets.push(
    ...
    return {
      tipo: 'Agresivo',
      motivo: 'Buen retorno, pero con riesgo, liquidez o plazo que exigen cautela.',
  ...
  if (
    ...
    return {
      tipo: 'Atractivo',
      motivo: 'Buen retorno con riesgo y liquidez controlados y plazo razonable.',
      bullets: bullets.slice(0, 5),
    };
  }
  ...
  return {
    tipo: 'Defensivo',
    motivo: 'Operación moderada: rentabilidad contenida y riesgo bajo.',
    bullets: bullets.slice(0, 5),
  };
}
...
export function evaluarCalidadAlternativo(a: AnalisisFinanciero, tienePlazo: boolean): CalidadDato {
  const defs: DefCampo[] = [
    ...
    },
    {
      ...
      peso: 3,
      critico: true,
      estado: (a) => (tiene(a.retorno_previsto_total) ? 'completo' : 'faltante'),
    },
    {
      label: 'Plazo / fechas',
      peso: 2,
    ...
    },
    {
      ...
      peso: 1,
      estado: (a) => (a.nivel_riesgo ? 'completo' : 'faltante'),
    },
    {
      label: 'Nivel de liquidez',
      peso: 1,
      estado: (a) => (a.nivel_liquidez ? 'completo' : 'faltante'),
    },
    {
      label: 'Descripción de la operación',
      peso: 1,
      estado: (a) => (a.descripcion_operacion ? 'completo' : 'estimado'),
    },
  ];
