...
import {
  evaluarCalidad, tieneValor as tiene,
  type Veredicto, type CalidadDato, type DefCampo,
} from './insights';
...
export function evaluarVeredictoRenta(
  ...
  if (van != null && van < 0) motivosRevisar.push('El VAN es negativo: no crea valor por encima de la tasa exigida.');
  if (cashflow != null && cashflow < 0) motivosRevisar.push('La caja anual es negativa: el activo no se autofinancia.');
  if (tieneDeuda && dscr != null && dscr < 1.1) motivosRevisar.push('El DSCR es bajo: la renta apenas cubre la cuota de deuda.');
  if (faltantesCriticos.length > 0) motivosRevisar.push(`Faltan datos clave: ${faltantesCriticos.slice(0, 3).join(', ')}.`);
  if (k.tirSinDatos) motivosRevisar.push('No hay datos suficientes (horizonte o valor residual) para una TIR fiable.');
  ...
  const residualPesoAlto = aporteResidual != null && van != null && van > 0 && aporteResidual > van * 0.5;
  ...
  if (tirSuperaTasa && ((dependeDelResidual && !operativaFuerte) || ltvAlto || dscrAjustado)) {
    if (dependeDelResidual && !operativaFuerte) bullets.push('La rentabilidad depende casi por completo del valor residual estimado: conviene validarlo.');
    if (ltvAlto && ltv != null) bullets.push(`Apalancamiento alto (LTV ${(ltv * 100).toFixed(0)}%): amplifica retorno y riesgo.`);
    if (dscrAjustado && dscr != null) bullets.push(`DSCR ajustado (${dscr.toFixed(2)}x): poco margen ante imprevistos.`);
    ...
    return { tipo: 'Agresivo', motivo: 'Buen retorno, pero apoyado sobre todo en el valor residual o en el apalancamiento.', bullets: bullets.slice(0, 5) };
  }
  ...
  if (tirSuperaTasa && vanPositivo && cashflowOk && dscrHolgado && funcionaPorRenta) {
    if (yieldNeto != null) bullets.push(`Rentabilidad neta sobre coste del ${(yieldNeto * 100).toFixed(1)}% (por encima de la tasa exigida del ${(tasa * 100).toFixed(1)}%).`);
    bullets.push('La caja anual es positiva: el activo se autofinancia.');
    if (van != null) bullets.push(`VAN positivo (${Math.round(van).toLocaleString('es-ES')} €)${residualPesoAlto ? ', con parte relevante aportada por el valor residual' : ''}.`);
    if (tieneDeuda && dscr != null) bullets.push(`DSCR holgado (${dscr.toFixed(2)}x): la renta cubre la deuda con margen.`);
    if (residualPesoAlto) bullets.push('Conviene validar el valor actual estimado, porque aporta una parte relevante del retorno total.');
  ...
  // ── Defensivo ───────────────────────────────────────────────────────────────
  bullets.push(yieldNeto != null ? `Rentabilidad neta moderada (${(yieldNeto * 100).toFixed(1)}% sobre coste).` : 'Rentabilidad moderada.');
  ...
  return { tipo: 'Defensivo', motivo: 'Perfil conservador: rentabilidad moderada y riesgo bajo.', bullets: bullets.slice(0, 5) };
}
...
export function evaluarCalidadRenta(a: AnalisisFinanciero): CalidadDato {
  ...
  const defs: DefCampo[] = [
    { label: 'Coste de compra',            peso: 3, critico: true,  estado: a => tiene(a.precio_adquisicion) ? 'completo' : 'faltante' },
    { label: 'Gastos de adquisición',      peso: 1,                 estado: a => tiene(a.gastos_adquisicion) ? 'completo' : 'faltante' },
    { label: 'CAPEX inicial',              peso: 1,                 estado: a => tiene(a.capex_inicial) ? 'completo' : 'faltante' },
    { label: 'Valor actual estimado',      peso: 2,                 estado: a => tiene(a.valoracion_actual) ? 'completo' : 'faltante' },
    { label: 'Renta mensual',              peso: 3, critico: true,  estado: a => tiene(a.renta_mensual_bruta) ? 'completo' : 'faltante' },
    { label: 'Ocupación',                  peso: 1,                 estado: a => a.tasa_ocupacion_prevista_pct != null ? 'completo' : 'estimado' },
    { label: 'Gastos operativos',          peso: 2,                 estado: a => tiene(a.gastos_operativos_anuales) ? 'completo' : 'faltante' },
    ...
    { label: 'Estructura de capital (deuda)', peso: 1,               estado: a => a.deuda_hipotecaria != null ? 'completo' : 'estimado' },
    ...
    { label: 'Método de valor residual',   peso: 2, critico: true,  estado: a => a.metodo_valor_residual != null ? 'completo' : 'estimado' },
  ...
  if (tieneDeuda) {
    defs.push(
      { label: 'Cuota / servicio de deuda', peso: 2, critico: true, estado: a => tiene(a.cuota_hipoteca_mensual) || tiene(a.tipo_interes_deuda_pct) ? 'completo' : 'faltante' },
      { label: 'Tipo de interés de deuda',  peso: 1,                estado: a => tiene(a.tipo_interes_deuda_pct) ? 'completo' : 'estimado' },
      { label: 'Plazo restante',            peso: 1,                estado: a => tiene(a.plazo_restante_anios) ? 'completo' : 'estimado' },
    );
  }
  ...
  if (a.aplicar_fiscalidad === true) {
    defs.push(
      { label: 'Tipo de IS',                peso: 1, estado: a => tiene(a.impuesto_sociedades_pct) ? 'completo' : 'estimado' },
      { label: 'Amortización fiscal',       peso: 1, estado: a => tiene(a.amortizacion_fiscal_anual) ? 'completo' : 'faltante' },
      { label: 'Costes de venta',           peso: 1, estado: a => tiene(a.costes_venta_pct) ? 'completo' : 'estimado' },
    );
  }
...
export function lecturaExplotarLiquidar(yieldNetoValorActual: number | null, tasa: number): ExplotarLiquidar {
  const nota = 'No es una recomendación de venta: hay que considerar impuestos, costes de venta, deuda, liquidez, revalorización esperada y alternativas disponibles.';
  if (yieldNetoValorActual == null) {
    return { nivel: 'na', titulo: 'Sin valor actual', mensaje: 'Falta el valor actual estimado para comparar explotación vs liquidación.', nota };
  }
