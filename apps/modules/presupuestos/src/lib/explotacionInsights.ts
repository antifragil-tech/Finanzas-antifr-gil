...
export function evaluarVeredictoExplotacion(
  ...
  const dependeTerminal =
    k.valorTerminalAplicado &&
    k.valorTerminalPresente != null &&
    van != null &&
    van > 0 &&
  ...
  if (sinInversion)
    motivosRevisar.push('Falta la inversión inicial: payback/VAN/TIR no son fiables.');
  if (ebitda != null && ebitda <= 0)
    motivosRevisar.push(
      'El EBITDA es nulo o negativo: el negocio no es rentable a nivel operativo.',
    );
  ...
  if (k.valorTerminalIncompleto)
    motivosRevisar.push(
      'El valor de continuidad está activado pero faltan datos (múltiplo o valor manual).',
    );
  ...
  const margenBueno = margen != null && margen >= 0.2;
  ...
  if (tirSuperaTasa && vanPositivo && (dependeTerminal || crecimientoAlto || paybackLargo)) {
    if (dependeTerminal)
      bullets.push(
        'Gran parte del valor descansa en el valor de continuidad (residual del negocio).',
      );
    if (crecimientoAlto)
      bullets.push(
        `Depende de un crecimiento anual exigente (${(k.crecimientoPct ?? 0).toFixed(0)}%).`,
      );
    ...
    return {
      tipo: 'Agresivo',
      motivo:
        'Buen retorno, pero muy dependiente de supuestos (valor de continuidad o crecimiento).',
  ...
  if (tirSuperaTasa && vanPositivo && margenBueno && paybackBueno) {
    ...
    if (payback != null)
      bullets.push(`Recuperación de la inversión en ${payback.toFixed(1)} años.`);
    if (!k.valorTerminalAplicado) bullets.push('Sin valor de continuidad: el VAN es conservador.');
    return {
      tipo: 'Atractivo',
      motivo: 'Negocio rentable con margen sólido y recuperación razonable.',
      bullets: bullets.slice(0, 5),
    };
  }
  ...
  bullets.push(
    margen != null ? `Margen EBITDA moderado (${(margen * 100).toFixed(1)}%).` : 'Margen moderado.',
  );
  if (payback != null) bullets.push(`Recuperación en ${payback.toFixed(1)} años.`);
  if (ebitda != null)
    bullets.push(`EBITDA positivo (${Math.round(ebitda).toLocaleString('es-ES')} €).`);
  if (van != null)
    bullets.push(
  ...
  return {
    tipo: 'Defensivo',
    motivo: 'Negocio estable: margen moderado y dependencia de supuestos baja.',
    bullets: bullets.slice(0, 5),
  };
}
...
export function evaluarCalidadExplotacion(a: AnalisisFinanciero): CalidadDato {
  const defs: DefCampo[] = [
    {
      ...
      peso: 3,
      critico: true,
      estado: (a) => (tiene(a.ingresos_anuales_previstos) ? 'completo' : 'faltante'),
    },
    {
      label: 'Costes fijos',
      peso: 3,
      critico: true,
      estado: (a) => (tiene(a.costes_fijos_anuales) ? 'completo' : 'faltante'),
    },
    {
      ...
      critico: true,
      estado: (a) =>
        tiene(a.precio_adquisicion) || tiene(a.inversion_circulante) ? 'completo' : 'faltante',
    },
    {
    ...
    },
    {
      ...
      peso: 2,
      estado: (a) => (a.costes_variables_pct != null ? 'completo' : 'estimado'),
    },
    {
    ...
    },
    {
      ...
      peso: 1,
      estado: (a) => (tiene(a.inversion_circulante) ? 'completo' : 'estimado'),
    },
    {
      label: 'Crecimiento anual',
      peso: 1,
      estado: (a) => (a.tasa_crecimiento_anual_pct != null ? 'completo' : 'estimado'),
    },
    {
    ...
    {
      label: 'EBITDA objetivo',
      peso: 1,
      estado: (a) => (tiene(a.ebitda_objetivo_anual) ? 'completo' : 'estimado'),
    },
  ];
  ...
  if (a.aplicar_fiscalidad === true) {
    defs.push({
      ...
      peso: 1,
      estado: (a) => (a.impuesto_sociedades_pct != null ? 'completo' : 'estimado'),
    });
  }
  ...
  if (a.aplicar_valor_terminal === true) {
    defs.push({
      ...
      estado: (a) =>
        (
          a.metodo_valor_terminal === 'multiplo_ebitda'
            ? tiene(a.multiplo_ebitda_salida)
            : tiene(a.valor_terminal_manual)
        )
          ? 'completo'
          : 'faltante',
    });
