...
> **Última verificación real:** 2026-07-04 ~13:35 (incluye estudio del contenido de todas las
> ramas). ⚠️ Los PRs **#4, #5 y #6 se crearon en paralelo por otros chats** durante esta misma
> sesión — recordatorio de que este tracker caduca rápido: verificar siempre antes de asumir.
...
| **Reservas / Agenda (F1)** | `feat/reservas-agenda-hoy` | `53c233a`+ ("Agenda v0.2") | F1 operativa standalone (`pnpm --filter @alsari/reservas dev`); **versión CANÓNICA del módulo (D7)**; el cableado host vive en demo (PR 5a) | Remoto ✅ + **PR #5 Draft (NO MERGE)** — creado en paralelo 2026-07-04 | — | Revisión; es el **GATE** (posición 4 del orden). Al mergear: archivar el spike |
...
| **Baseline Supabase** | `chore/db-baseline-antifragil-os` | `6c49503`+ | Baseline curado en `services/supabase/baselines/antifragil_os/`: SQL 26 tablas + RLS + Storage + seed mínimo (Antifrágil S.C. `ANT`, proyecto `CLI-PLY` activo, emails placeholder) + runbook + checks + `excluded_legacy.md`. **Nada aplicado** | Remoto ✅ + **PR #4 Draft (NO APPLY)** — creado en paralelo 2026-07-04 | Supabase nuevo no creado | Revisión (posición 6 del orden; additive, sin aplicar SQL) |
| **A1 Tesorería/Caja** | `chore/db-baseline-antifragil-os` (subcarpeta `a1_tesoreria/`) | `6c49503` | Draft encima del baseline: `cuenta_tesoreria`, `movimiento_caja` (append-only), `arqueo_caja`, vistas de tesorería, 3 RPCs, seed "Caja Clínica Playamar". Puente pagos→tesorería **diferido a A1b**. Implementa el mini-diseño FOP-A1 (doc 06 de finanzas) | Con baseline | Baseline aplicado primero (futuro) | Revisar dentro del PR de baseline |
...
| **QA** | `qa/smoke-suite-antifragil-os` | `4e4272c` | Suite no destructiva (Chat 5): `smoke-routes.mjs`, `check-no-secrets.mjs`, `check-legacy-strings.mjs` + 5 checklists + runbook en `docs/qa/` | Remoto ✅ + **PR #6 Draft** — creado en paralelo 2026-07-04 | — | Revisión (aditivo, puede ir temprano) |
