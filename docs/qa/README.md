...
- **No secretos**: que no haya claves, JWT, `service_role`, `DB_PASSWORD`, `.env` reales en el diff.
- **Estado de ramas**: apoyo a la revisión de PRs (checklist).
- **Baseline Supabase**: checklist de QA (sin aplicar SQL).
...
- **`check-no-secrets` encuentra algo** → **PARAR**. No subir. Reportar el hallazgo **sin copiar el secreto** (el script ya enmascara). Rotar la clave si era real.
...
- ❌ No hacer push/PR/merge/rebase desde esta rama de QA.
- ❌ No usar datos reales: todo lo que se prueba es mock/demo.
...
| `docs/qa/checklist-supabase-baseline.md` | manual | revisar baseline (sin aplicar SQL) |
