...
- **Descripción:** `packages/supabase-client/src/index.ts` contiene un **fallback hardcodeado**: URL del proyecto legacy + una **`FALLBACK_ANON_KEY`** (JWT anon real). Confirmado también por la suite QA (PR #6) como hallazgo preexistente. *(No se reproduce la clave en esta documentación.)*
