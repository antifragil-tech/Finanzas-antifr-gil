        ...
        if (url.pathname.match(/^\/api\/invoices\/[^/]+\/action$/) && request.method === 'PATCH') {
            try {
                ...
                if (body.action === 'accept_direccion' && oldStatus === 'pending') {
                    ...
                    return errorResponse(`Invalid action '${body.action}' for status '${oldStatus}'`, request, 400);
                }
        ...
        if (url.pathname.match(/^\/api\/invoices\/[^/]+\/finalize$/) && request.method === 'POST') {
            try {
                ...
                if (invoice.status !== 'en_finanzas') {
                    return errorResponse(`La factura no está en estado 'en_finanzas' (actual: ${invoice.status})`, request, 400);
                }
...
Devuelve SOLO el JSON sin markdown. Sé flexible con variaciones menores del nombre.`;
                ...
                allLogs.push(`[VALIDATION] Importe justificante: ${validation.importe} | Factura: ${invoiceAmount}`);
                allLogs.push(`[VALIDATION] Beneficiario: ${validation.beneficiario} | Proveedor: ${invoiceProvider}`);

                // Check match
                if (!validation.coincide_importe || !validation.coincide_beneficiario) {
                    const reasons: string[] = [];
                    if (!validation.coincide_importe) {
                        reasons.push(`Importe: justificante ${validation.importe}€ vs factura ${invoiceAmount}€`);
                    }
                    if (!validation.coincide_beneficiario) {
                        reasons.push(`Beneficiario: justificante "${validation.beneficiario}" vs factura "${invoiceProvider}"`);
                    }
                    return jsonResponse({
                        success: false,
                ...
                const dateStr = invoiceDate.includes('/')
                    ...
                    : invoiceDate;
                const safeName = invoiceProvider.replace(/[^a-zA-Z0-9áéíóúñÁÉÍÓÚÑ ]/g, '').replace(/\s+/g, '_');
                ...
                // Upload proof
                const proofExt = proofMimeType.includes('pdf') ? 'pdf' : proofMimeType.split('/')[1] || 'pdf';
        ...
        if (url.pathname.startsWith('/api/invoices/') && url.pathname.endsWith('/pdf') && request.method === 'GET') {
            try {
                const parts = url.pathname.split('/');
