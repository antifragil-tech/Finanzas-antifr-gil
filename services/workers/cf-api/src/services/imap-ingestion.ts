...
export class ImapIngestionService {
    ...
    async fetchNewInvoices(): Promise<any[]> {
        ...
        try {
            ...
            this.log(`📥 Encontrados ${uids.length} correos potenciales. Descargando uno a uno para máxima fiabilidad...`);
            ...
            for (let i = 0; i < uids.length; i++) {
                ...
                const isTargetRecipient = to.toLowerCase().includes('finanzas@alsari.net') ||
                ...
                if (parsed.attachments && parsed.attachments.length > 0) {
                    ...
                    for (const att of parsed.attachments) {
                        ...
                        const isPDF = att.mimeType === 'application/pdf' || filename.toLowerCase().endsWith('.pdf');
                        if (isPDF) {
                            ...
                            try {
                                ...
                                this.log(`   [DEBUG] Attachment content type: ${att.content?.constructor?.name}, size: ${uint8.byteLength} bytes`);
                                this.log(`   [DEBUG] First 20 bytes: [${Array.from(uint8.slice(0, 20)).join(', ')}]`);
                                this.log(`   [DEBUG] PDF header check: ${String.fromCharCode(...uint8.slice(0, 5))}`);
                                ...
                                this.log(`   [DEBUG] Base64 length: ${base64Content.length}, preview: ${base64Content.substring(0, 30)}...`);
                                ...
                                const sanitizedProvider = (data.proveedor || 'unknown').replace(/[^a-z0-9]/gi, '').toUpperCase();
...
Devuelve SOLO el JSON, sin markdown ni explicaciones.`;
        ...
        for (let attempt = 0; attempt <= delays.length; attempt++) {
            ...
            if (response.status === 429 && attempt < delays.length) {
                ...
                this.log(`   ⏳ Gemini cuota agotada. Reintentando en ${waitSec}s... (intento ${attempt + 1}/${delays.length})`);
