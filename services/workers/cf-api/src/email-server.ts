...
const server = createServer(async (req, res) => {
    ...
    if (req.method === 'POST' && req.url === '/send') {
        try {
            ...
            const info = await transporter.sendMail({
                from: from || `"Alsari Capital" <${process.env.IMAP_USER || 'guille.vila.marcos@gmail.com'}>`,
                to, subject, html,
            });
    ...
    if (req.method === 'POST' && req.url === '/send-with-attachments') {
        try {
            ...
            console.log(`✅ Email con ${mailAttachments.length} adjuntos enviado a ${to} (${info.messageId})`);
            jsonResponse(res, 200, { success: true, messageId: info.messageId });
        } catch (err: any) {
    ...
    if (req.method === 'POST' && req.url === '/upload-to-drive') {
        try {
            ...
            console.log(`[DRIVE] 📤 Subiendo: ${fileName} (${(fileBuffer.length / 1024).toFixed(1)} KB) → ${folderId}`);
    ...
    // Health check
    jsonResponse(res, 200, { status: 'ok', service: 'alsari-email-drive-sidecar', hasDriveCredentials: !!GOOGLE_REFRESH_TOKEN });
});
...
server.listen(PORT, () => {
    ...
    console.log(`   Drive OAuth2: ${GOOGLE_REFRESH_TOKEN ? '✅ Configurado' : '❌ FALTA GOOGLE_REFRESH_TOKEN'}`);
});
